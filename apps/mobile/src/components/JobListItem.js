					...
					<View style={styles.cardHeader}>
						<Text
							style={[
								...
								item.status === "COMPLETED" && styles.textStrike,
							]}
						>
							{item.title}
						</Text>
						<View style={styles.headerRight}>
							{item.status !== "COMPLETED" &&
								renderPriorityDot(item.priority, theme)}
					...
					<View style={styles.cardFooter}>
						<View style={styles.footerInfo}>
							...
							<View style={styles.infoRow}>
								...
								>
									Atanan:{" "}
									{item.assignments?.[0]?.team?.name ||
										item.assignments?.[0]?.worker?.name ||
										item.assignee?.name ||
										"Atanmamış"}
								</Text>
							</View>
