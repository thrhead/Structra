			...
			<View style={styles.modalOverlay}>
				...
				>
					<Text style={[styles.modalTitle, { color: theme.colors.text }]}>
						{initialData ? t("manager.editTeam") : t("manager.addTeam")}
					</Text>
					...
					>
						...
						>
								...
								.map((u) => (
									<TouchableOpacity
										...
										onPress={() =>
											setFormData({
												...formData,
												leadId: formData.leadId === u.id ? "" : u.id,
											})
										}
									>
										<View
							...
							{availableUsers.filter(
								(u) => u.role === "TEAM_LEAD" || u.role === "MANAGER",
							).length === 0 && (
								<Text
									style={[styles.emptyText, { color: theme.colors.subText }]}
								>
									{t("manager.noLeadOrAdmin")}
								</Text>
							)}
						</View>
						...
						>
							{availableUsers
								.filter((u) => u.role === "WORKER" || u.role === "TEAM_LEAD")
								.map((u) => (
									<TouchableOpacity
										key={u.id}
										style={styles.checkboxItem}
							...
							{availableUsers.filter(
								(u) => u.role === "WORKER" || u.role === "TEAM_LEAD",
							).length === 0 && (
								<Text
									style={[styles.emptyText, { color: theme.colors.subText }]}
								>
