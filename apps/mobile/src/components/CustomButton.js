...
const CustomButton = ({
	...
	const slate700 = theme
		? theme.colors.border || COLORS.slate700
		: COLORS.slate700;
	const slate500 = theme
		? theme.colors.subText || COLORS.slate500
		: COLORS.slate500;
	const slate400 = theme
		? theme.colors.subText || COLORS.slate400
		: COLORS.slate400;
	const white = theme ? theme.colors.textInverse || COLORS.white : COLORS.white;
	const _black = theme
		? theme.colors.textInverse || theme.colors.textInverse
		...
		<Pressable
			...
			style={({ pressed }) => [
				styles.button,
				{
					...
					opacity:
						pressed && (variant === "primary" || variant === "danger")
							? 0.8
							: 1,
