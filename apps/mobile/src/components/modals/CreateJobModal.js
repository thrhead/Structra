...
export default function CreateJobModal({ visible, onClose, onSuccess }) {
	...
	const handleCreateJob = async () => {
		...
		try {
			...
			const msg =
				error.response?.data?.error || "İş oluşturulurken bir hata oluştu.";
		...
		if (target === "customer") {
			...
			if (customers.length === 0) {
				try {
					...
					setValidationError("Müşteriler yüklenemedi.");
				}
			} else {
				setSelectionItems(
					customers.map((c) => ({
			...
			if (teams.length === 0) {
				try {
					...
					const options = [
						{ id: null, label: t("jobs.noAssignment") },
						...teamData.map((t) => ({ id: t.id, label: t.name })),
					];
					setSelectionItems(options);
				} catch (_e) {
					setValidationError("Ekipler yüklenemedi.");
				}
			} else {
				const options = [
					{ id: null, label: t("jobs.noAssignment") },
				...
				<View style={styles.modalContent}>
					...
					<ScrollView style={{ maxHeight: 500 }}>
						<Text style={styles.label}>
							{t("jobs.template")} {t("jobs.optional")}
						</Text>
						<TouchableOpacity
						...
						>
							<Text style={styles.selectorButtonText}>
								{formData.templateName || t("jobs.selectTemplate")}
							</Text>
						...
						<TextInput
							style={[
								...
								loading && { opacity: 0.5 },
								validationError &&
									!formData.title && { borderColor: "#ff4444" },
							]}
						...
						<TouchableOpacity
							style={[
								...
								loading && { opacity: 0.5 },
								validationError &&
									!formData.customerId && { borderColor: "#ff4444" },
							]}
							onPress={() => openSelection("customer")}
							disabled={loading}
						>
							<Text style={styles.selectorButtonText}>
								{formData.customerName || t("jobs.selectCustomer")}
							</Text>
							<MaterialIcons name="arrow-drop-down" size={24} color="#aaa" />
						</TouchableOpacity>

						<Text style={styles.label}>
							{t("jobs.assignTeam")} {t("jobs.optional")}
						</Text>
						<TouchableOpacity
							style={[styles.selectorButton, loading && { opacity: 0.5 }]}
						...
						>
							<Text style={styles.selectorButtonText}>
								{formData.teamName || t("jobs.selectTeam")}
							</Text>
							<MaterialIcons name="arrow-drop-down" size={24} color="#aaa" />
						</TouchableOpacity>
						...
						<View style={{ flexDirection: "row", gap: 10 }}>
							...
							<View style={{ flex: 1 }}>
								...
								>
									<Text style={styles.selectorButtonText}>
										{formData.scheduledEndDate.toLocaleDateString()}
									</Text>
								</TouchableOpacity>
							</View>
						</View>
							...
							<FlatList
								...
								renderItem={({ item }) => (
									...
									>
										...
										{item.sub && (
											<Text style={styles.selectionItemSub}>{item.sub}</Text>
										)}
									</TouchableOpacity>
								)}
							/>
