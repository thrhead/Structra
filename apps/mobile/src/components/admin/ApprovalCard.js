...
const ApprovalCard = ({ item, onApprove, onReject, theme: propTheme }) => {
	...
	return (
		...
		>
			...
			>
				<View
					style={[
						...
						{
							backgroundColor:
								item.type === "JOB" ? `${primary}15` : "rgba(255, 165, 0, 0.1)",
						},
					]}
				>
					<MaterialIcons
			...
			<Text style={[styles.requester, { color: textSub }]}>
				{item.requester}
			</Text>
					...
					{item.type === "COST" ? (
						<>
							...
							{item.raw?.notes && (
								<View style={styles.detailRow}>
									...
									>
										<Text style={[styles.noteText, { color: textMain }]}>
											{item.raw?.notes}
										</Text>
									</View>
								</View>
						...
						<>
							<View style={styles.detailRow}>
								...
								<Text style={[styles.detailValue, { color: textMain }]}>
									{item.raw?.job?.customer?.company ||
										item.raw?.customer?.company ||
										"Bilinmiyor"}
								</Text>
							</View>
							<View style={styles.detailRow}>
								...
								<Text style={[styles.detailValue, { color: textMain }]}>
									{item.raw?.job?.customer?.address ||
										item.raw?.customer?.address ||
										"Bilinmiyor"}
								</Text>
							</View>
						</>
			...
			<View style={styles.actions}>
				<TouchableOpacity
					style={[
						styles.actionButton,
						styles.rejectButton,
						{ borderColor: `${error}4D`, backgroundColor: `${error}15` },
					]}
				...
				>
					...
					/>
					<Text
						style={[styles.approveText, { color: theme.colors.textInverse }]}
					>
						Onayla
					</Text>
				</TouchableOpacity>
