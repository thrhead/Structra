	...
	return (
		<View
			style={[
				...
				{ backgroundColor: theme ? theme.colors.background : "#0f172a" },
			]}
		>
			...
			<ScrollView style={styles.eventsList}>
				{events.length === 0 ? (
					<Text style={[styles.noEvents, { color: textSub }]}>
						Bu tarihte planlanmış iş yok
					</Text>
				) : (
					events.map((event) => (
						<TouchableOpacity
						...
						>
							...
							{event.location && (
								<Text style={[styles.eventDetail, { color: textSub }]}>
									📍 {event.location}
								</Text>
							)}
							{event.assignments && (
