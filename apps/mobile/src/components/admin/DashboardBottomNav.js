...
const DashboardBottomNav = ({ navigation, activeTab = "Dashboard" }) => {
	...
	const animatedValues = {
		Dashboard: useRef(new Animated.Value(activeTab === "Dashboard" ? 1.2 : 1))
			.current,
		TeamManagement: useRef(
			new Animated.Value(activeTab === "TeamManagement" ? 1.2 : 1),
		).current,
		Jobs: useRef(new Animated.Value(activeTab === "Jobs" ? 1.2 : 1)).current,
		Profile: useRef(new Animated.Value(activeTab === "Profile" ? 1.2 : 1))
			.current,
	};
			...
			<View style={styles.navInner}>
				{navItems.map((item) => {
					...
					return (
						...
						>
							<Animated.View
								style={{ transform: [{ scale: animatedValues[item.id] || 1 }] }}
							>
								<Icon
									size={24}
