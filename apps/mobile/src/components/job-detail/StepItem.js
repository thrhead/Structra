...
const StepItem = ({
	...
	const getStatusText = () => {
		if (step.approvalStatus === "APPROVED")
			return t("common.approved") || "ONAYLANDI";
		if (step.approvalStatus === "REJECTED")
			return t("common.rejected") || "REDDEDİLDİ";
		...
		>
					...
					<View style={styles.titleRow}>
						...
						>
							{step.stepNo && (
								...
								>
									<Text
										style={[styles.stepNoText, { color: theme.colors.primary }]}
									>
										{step.stepNo}
									</Text>
								</View>
					...
					<View style={styles.datesContainer}>
						{step.startedAt && (
							<Text style={[styles.dateText, { color: theme.colors.subText }]}>
								<MaterialIcons name="play-circle-outline" size={14} />{" "}
			...
			{step.approvalStatus === "REJECTED" && step.rejectionReason && (
				...
				>
					...
					/>
					<Text
						style={[styles.rejectionReasonText, { color: theme.colors.error }]}
					>
			...
			{isManager && isCompleted && step.approvalStatus === "PENDING" && (
				<View style={styles.managerActionRow}>
					...
					>
						...
						<Text
							style={[styles.managerButtonText, { color: theme.colors.error }]}
						>
					...
					>
						...
						<Text style={[styles.managerButtonText, { color: "#fff" }]}>
			...
			{!isLocked && step.subSteps && step.subSteps.length > 0 && (
				<View style={styles.substepsWrapper}>
					...
					/>
					<View style={styles.substepsContainer}>{children}</View>
				</View>
			)}
		</View>
	);
