...
const jobService = {
	...
	rejectJob: async (jobId, reason) => {
		const response = await api.post(`/api/manager/jobs/${jobId}/reject`, {
			reason,
		});
		return response.data;
	},
	...
	assignJob: async (jobId, workerId) => {
		const response = await api.post(`/api/admin/jobs/${jobId}/assign`, {
			workerId,
		});
		return response.data;
	},
	...
	startStep: async (jobId, stepId, updatedAt) => {
		const response = await api.post(
			`/api/worker/jobs/${jobId}/steps/${stepId}/start`,
			{ updatedAt },
		);
		return response.data;
	},
	...
	rejectStep: async (stepId, reason) => {
		const response = await api.post(`/api/manager/steps/${stepId}/reject`, {
			reason,
		});
		return response.data;
	},
	...
	startSubstep: async (_jobId, _stepId, substepId) => {
	...
	approveSubstep: async (substepId) => {
		const response = await api.post(
			`/api/manager/substeps/${substepId}/approve`,
		);
		return response.data;
	},
	...
	rejectSubstep: async (substepId, reason) => {
		const response = await api.post(
			`/api/manager/substeps/${substepId}/reject`,
			{ reason },
		);
		return response.data;
	},
	...
	deletePhoto: async (jobId, stepId, photoId) => {
		const response = await api.delete(
			`/api/worker/jobs/${jobId}/steps/${stepId}/photos/${photoId}`,
		);
		return response.data;
	},
	...
	uploadAudio: async (jobId, stepId, formData, subStepId = null) => {
		...
		const response = await api.post(
			`/api/worker/jobs/${jobId}/steps/${stepId}/audio`,
			formData,
			{
				headers: {
					Accept: "application/json",
