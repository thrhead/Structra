...
const MEDIA_DIR =
	Platform.OS === "web" ? "" : `${FileSystem.documentDirectory}offline_media/`;
	...
	getItems: async () => {
		try {
			...
			try {
				...
				console.error(
					"[QueueService] JSON parse error, clearing corrupted queue:",
					parseError,
				);
	...
	removeItem: async (id) => {
		try {
			...
			if (Platform.OS !== "web" && itemToRemove?.payload?._localUri) {
				try {
					await FileSystem.deleteAsync(itemToRemove.payload._localUri, {
						idempotent: true,
					});
				} catch (e) {
