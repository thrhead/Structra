...
export const LoggerService = {
	...
	init: () => {
		...
		if (typeof global !== "undefined") {
			...
			ErrorUtils.setGlobalHandler((error, isFatal) => {
				LoggerService.error(
					`Unhandled Error (${isFatal ? "Fatal" : "Non-fatal"}): ${error.message}`,
					null,
					error.stack,
				);
				if (originalHandler) {
	...
	error: (msg, ctx, stack) =>
		LoggerService.log(LogLevel.ERROR, msg, ctx, stack),
	...
	sync: async () => {
		...
		try {
			...
			if (response.status === 201) {
				...
				console.warn(
					`[LoggerService] Server responded with status ${response.status}. Keeping logs.`,
				);
			}
		} catch (error) {
