...
export const SyncManager = {
	...
	init: () => {
		...
		SyncManager.appStateSubscription = AppState.addEventListener(
			"change",
			(nextAppState) => {
	...
	destroy: () => {
		...
		if (SyncManager.appStateSubscription)
			SyncManager.appStateSubscription.remove();
	},
		...
		try {
			...
			for (const item of sortedQueue) {
				if (item.retryCount >= 3) {
					console.warn(
						`[SyncManager] Item ${item.id} reached max retries. Skipping.`,
					);
					continue;
				}
				...
				try {
					...
					if (currentPayload._localUri) {
						try {
							const fileContent = await FileSystem.readAsStringAsync(
								currentPayload._localUri,
							);
							currentPayload.photo = fileContent;
							delete currentPayload._localUri;
						} catch (fileError) {
							console.error(
								"[SyncManager] Local file not found, skipping item:",
								fileError,
							);
					...
					if (error.status === 409) {
						console.error(
							`[SyncManager] Conflict detected for item ${item.id}. Manual intervention required.`,
						);
						ToastService.show(
							"Veri Çakışması",
							"Bu işlem sunucudaki güncel veriyle çakışıyor.",
							"warning",
						);
					...
					if (item.retryCount >= 3) {
						ToastService.show(
							...
							"Bazı veriler sunucuya gönderilemedi.",
							"error",
						);
					}
				}
			...
			if (successCount > 0) {
				ToastService.show(
					...
					`${successCount} işlem sunucuya gönderildi.`,
					"success",
				);
			}
		} catch (error) {
