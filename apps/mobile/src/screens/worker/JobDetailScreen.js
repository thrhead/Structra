...
export default function JobDetailScreen({ route, navigation }) {
	...
	const handleDeletePhoto = (photo) => {
		...
		showAlert(t("common.warning"), warningMsg, [
	...
	const handleSubstepToggle = async (stepId, substepId, currentStatus) => {
		const performToggle = async () => {
			try {
				...
				if (!currentStatus) {
					...
					const hasPhotos =
						substep?.photos &&
						Array.isArray(substep.photos) &&
						substep.photos.length > 0;
				...
				await jobService.toggleSubstep(
					jobId,
					stepId,
					substepId,
					!currentStatus,
		...
		if (substep?.approvalStatus === "APPROVED") {
			showAlert(t("common.warning"), t("alerts.approvedActionWarning"), [
				{ text: t("common.cancel"), style: "cancel" },
				{
			...
			try {
				...
				if (source === "camera") {
					...
					if (status !== "granted") {
						showAlert(
							t("alerts.permissionRequired"),
							t("alerts.galleryPermissionDesc"),
							[],
							"warning",
						);
						return;
				...
				if (!result.canceled) {
					...
					uploadPhoto(
						stepId,
						substepId,
						optimized ? optimized.uri : result.assets[0].uri,
						optimized ? optimized.base64 : result.assets[0].base64,
					);
				}
			} catch (error) {
	...
	const handleRejectStep = async () => {
		if (!rejectionReason) {
			showAlert(
				t("common.warning"),
				t("alerts.rejectionReasonRequired"),
				[],
				"warning",
			);
			return;
	...
	const handleDeleteJob = async () => {
		showAlert(
			...
			t("common.confirmDelete") ||
				"Bu işi tamamen silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.",
			[
				{ text: t("common.cancel"), style: "cancel" },
				{
					text: t("common.delete"),
	...
	const handleStartJob = async () => {
		try {
			...
			LoggerService.audit("Job started", {
				jobId,
				jobTitle: job.title,
				workerId: user.id,
			});
			...
			showAlert(
				t("common.error"),
				error.message || t("alerts.jobStartError"),
				[],
				"error",
			);
		} finally {
		...
		try {
			...
			const allStepsCompleted =
				...
				job.steps.every((step) => {
					...
					const altAdimlarTamam =
						!step.subSteps ||
						step.subSteps.length === 0 ||
						step.subSteps.every(
							(ss) => ss.isCompleted === true || ss.status === "COMPLETED",
						);
			...
			if (!allStepsCompleted) {
				showAlert(
					t("common.warning"),
					t("alerts.photoRequiredForComplete"),
					[],
					"warning",
				);
				return;
	...
	const handleConfirmComplete = async () => {
		try {
			...
			LoggerService.audit("Job completed by worker", {
				jobId,
				jobTitle: job.title,
			});
	...
	const handleExportProforma = async () => {
		try {
			...
			const response = await axios.get(
				`${API_URL}/api/v1/jobs/${jobId}/proforma`,
				{
			...
			>
				{job && (
					<>
						...
						{["ADMIN", "MANAGER"].includes(user?.role?.toUpperCase()) &&
							job.status === "PENDING_APPROVAL" && (
								...
								>
									...
									<Text
										style={[styles.statusText, { color: theme.colors.subText }]}
									>
						...
						<CostSection
							job={job}
							canAdd={["WORKER", "TEAM_LEAD"].includes(
						...
						<GlassCard style={styles.responsibleCard} theme={theme}>
							<View style={styles.responsibleInfo}>
								...
								<View style={styles.responsibleTextContent}>
									...
									<Text
										style={[
											styles.responsibleRole,
											{ color: theme.colors.subText },
										]}
									>
										{t("common.mainResponsible")}
									</Text>
								</View>
							</View>
						</GlassCard>
									...
									<View style={styles.assignmentHeader}>
										...
										>
											{assignment.team
												...
												: assignment.worker?.name || t("common.team")}
										</Text>
									</View>
									{assignment.team?.members &&
										assignment.team.members.length > 0 && (
