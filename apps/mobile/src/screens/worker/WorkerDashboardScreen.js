...
export default function WorkerDashboardScreen({ navigation }) {
	...
	const loadDashboardData = useCallback(async () => {
		try {
			...
			const active = jobs
				? jobs.filter((j) => ["PENDING", "IN_PROGRESS"].includes(j.status))
				: [];
			...
			const formattedActiveJobs = active.map((job) => {
				...
				return {
					...
					title: job.title,
					location:
						job.location || job.customer?.company || "Konum belirtilmemiş",
					time: job.scheduledDate
						? new Date(job.scheduledDate).toLocaleTimeString("tr-TR", {
								hour: "2-digit",
								minute: "2-digit",
							})
						: "Saat belirtilmemiş",
		...
		<View style={styles.header}>
			<View style={styles.headerContent}>
				<View style={styles.headerTop}>
					<View style={styles.profileSection}>
						...
						>
							...
							>
								{user?.name ? user.name.charAt(0).toUpperCase() : "Ç"}
							</Text>
						</View>
						<View>
							<Text
								style={[
									styles.greetingText,
									{
										color: isDark ? "rgba(255,255,255,0.6)" : theme.colors.text,
									},
								]}
							>
								Merhaba,
				...
				<Text
					style={[
						styles.dateText,
						{ color: isDark ? "rgba(255,255,255,0.5)" : theme.colors.text },
					]}
				>
	...
	const getFilteredJobs = () => {
		...
		if (activeFilter === "IN_PROGRESS")
			return activeJobs.filter((job) => job.status === "In Progress");
							...
							<View style={styles.sectionContainer}>
								...
								>
									<View style={styles.costHeader}>
										<View>
											<Text
												style={[styles.costTitle, { color: theme.colors.text }]}
											>
												Masraflar
											</Text>
											<Text
...
const styles = StyleSheet.create({
	...
	heroCircleContainer: {
		...
		height: 300,
		overflow: "hidden",
	}, // Placeholder for HTML circle decorations
	tasksList: {
		gap: 16,
	},
	taskCard: {
