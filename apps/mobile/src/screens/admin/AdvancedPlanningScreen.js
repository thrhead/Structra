...
const AdvancedPlanningScreen = () => {
	...
	const groupedJobs = useMemo(() => {
		...
		filteredJobs.forEach((job) => {
			const date = new Date(job.start).toLocaleDateString(
	...
	const renderStatusBadge = (status) => {
		const colors = {
			PLANNED: {
				...
				text: "#3B82F6",
				label: t("manager.planned"),
			},
			ASSIGNED: {
				bg: "rgba(99, 102, 241, 0.1)",
				text: "#6366F1",
				label: t("manager.assigned"),
			},
			IN_PROGRESS: {
				bg: "rgba(245, 158, 11, 0.1)",
				text: "#F59E0B",
				label: t("manager.inProgress"),
			},
			COMPLETED: {
	...
	return (
		...
		>
			{/* Header / Tabs */}
			<View
				style={[styles.header, { borderBottomColor: theme.colors.cardBorder }]}
			>
					...
					filteredJobs.map((job) => (
						...
						>
							...
							<View style={styles.infoRow}>
								...
								<Text
									style={[styles.infoText, { color: theme.colors.subText }]}
								>
									{job.customer}
								</Text>
							</View>
