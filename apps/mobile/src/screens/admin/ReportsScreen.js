  ...
  const fetchReports = useCallback(async (isMounted = true) => {
    try {
      ...
      const [dashboardRes, exportsRes] = await Promise.all([
        ...
        axios
          .get(`${API_URL}/api/admin/reports/list`, { headers })
          .catch(() => ({ data: [] })),
      ]);
    ...
    <GlassCard style={styles.teamCard} theme={theme}>
      <View style={styles.teamHeader}>
        ...
        >
          <Zap
            size={14}
            color={
              item.stats.efficiencyScore > 80 ? "#22c55e" : theme.colors.primary
            }
          />
          <Text
            style={[
      ...
      <View style={styles.teamStatsRow}>
        <View style={styles.teamStat}>
          ...
          <Text style={[styles.teamStatVal, { color: theme.colors.text }]}>
            {item.stats.completedJobs}/{item.stats.totalJobs}
          </Text>
        </View>
        <View style={styles.teamStat}>
          <Text style={styles.teamStatLabel}>GİDER</Text>
          <Text style={[styles.teamStatVal, { color: theme.colors.text }]}>
            ₺{item.stats.totalExpenses.toLocaleString("tr-TR")}
          </Text>
        </View>
        <View style={styles.teamStat}>
          <Text style={styles.teamStatLabel}>SÜRE</Text>
          <Text style={[styles.teamStatVal, { color: theme.colors.text }]}>
            {item.stats.totalWorkingHours}s
          </Text>
        </View>
      </View>
      ...
      <View style={styles.progressBarBg}>
        <View
          style={[
            ...
            {
              width: `${item.stats.efficiencyScore}%`,
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={{ color: theme.colors.subText, fontSize: 14 }}>
          {selectedTemplate.toUpperCase()} Görünümü Aktif
        </Text>
      </View>
      ...
      <View style={{ marginBottom: 24 }}>
        <Text
          style={[
            styles.sectionTitle,
            { color: theme.colors.text, marginLeft: 20, marginBottom: 12 },
          ]}
        >
          Rapor Taslağı
        </Text>
          ...
          {templates.map((temp) => (
            ...
            >
              <temp.icon
                size={16}
                color={
                  selectedTemplate === temp.id ? "#fff" : theme.colors.primary
                }
              />
              <Text
                style={[
                  ...
                  {
                    color:
                      selectedTemplate === temp.id ? "#fff" : theme.colors.text,
                  },
                ]}
              >
                {temp.title}
            ...
            <View style={styles.statsGrid}>
              <GlassCard style={styles.statCard} theme={theme}>
                ...
                <Text style={[styles.statVal, { color: theme.colors.text }]}>
                  %{strategicData?.overallProfitMargin?.toFixed(1)}
                </Text>
                <Text
          ...
          <View style={styles.animateContent}>
            <View style={styles.globalStatsRow}>
              <View style={styles.globalStatItem}>
                <Text
                  style={[
                    styles.globalStatVal,
                    { color: theme.colors.primary },
                  ]}
                >
                  %{tacticalData?.avgTeamLoad?.toFixed(0)}
                </Text>
              ...
              <View style={styles.globalStatItem}>
                ...
                >
                  {tacticalData?.teamCapacity?.length}
                </Text>
          ...
          <View style={styles.animateContent}>
            <View style={styles.statsGrid}>
              <GlassCard style={styles.statCard} theme={theme}>
                ...
                <Text style={[styles.statVal, { color: theme.colors.text }]}>
                  {operationalData?.pendingApprovals?.costs +
                    operationalData?.pendingApprovals?.steps}
                </Text>
                <Text
                  style={[styles.statLabel, { color: theme.colors.subText }]}
                >
              ...
              <GlassCard style={styles.statCard} theme={theme}>
                ...
                <Text style={[styles.statVal, { color: theme.colors.text }]}>
                  %{operationalData?.bottleneckScore?.toFixed(0)}
                </Text>
                <Text
                  style={[styles.statLabel, { color: theme.colors.subText }]}
                >
            ...
            {exportsData && exportsData.length > 0 ? (
              ...
              <View style={{ padding: 40, alignItems: "center" }}>
                ...
                <Text style={{ color: theme.colors.subText, fontSize: 16 }}>
                  İndirilebilir rapor bulunamadı.
                </Text>
              </View>
            )}
          </View>
