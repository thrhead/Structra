...
const ReportsScreen = () => {
    ...
    const fetchReports = useCallback(async (isMounted = true) => {
        try {
            ...
            // Fix: perfRes was missing in previous branch code
            const operational = dashboardRes.data.operational;
            if (operational?.weeklySteps?.currentWeek?.length > 0) {
                setSelectedDay(operational.weeklySteps.currentWeek[operational.weeklySteps.currentWeek.length - 1]);
            }
        } catch (error) {
