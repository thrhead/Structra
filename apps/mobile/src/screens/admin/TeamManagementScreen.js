...
const TeamManagementScreen = ({ navigation }) => {
	...
	const renderTeamItem = ({ item }) => (
		...
		>
				...
				<View style={styles.cardBody}>
					<View style={styles.statRow}>
						...
						<Text style={[styles.statText, { color: theme.colors.subText }]}>
							{t("manager.memberCount", {
								count: item.memberCount || item.members?.length || 0,
							})}
						</Text>
					</View>
			...
			{isAdmin && (
				<View
					style={[styles.actions, { borderTopColor: theme.colors.cardBorder }]}
				>
					<TouchableOpacity
				...
				<View style={styles.modalOverlay}>
					...
					>
						...
						<Text style={[styles.deleteText, { color: theme.colors.text }]}>
							{teamToDelete?.name}{" "}
							{t("profile.logoutConfirmDesc").includes("?") ? "?" : ""}
						</Text>
