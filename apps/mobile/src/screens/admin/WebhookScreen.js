...
export default function WebhookScreen() {
	...
	const renderEndpointItem = ({ item }) => (
		<GlassCard theme={theme} style={styles.webhookCard}>
			<View style={styles.cardHeader}>
				<View style={{ flex: 1 }}>
					<Text
						style={[styles.webhookUrl, { color: theme.colors.text }]}
						numberOfLines={1}
					>
						{item.url}
					</Text>
					<Text style={[styles.webhookEvent, { color: theme.colors.primary }]}>
						{item.event}
					</Text>
				</View>
				<Switch
			...
			<View style={styles.cardFooter}>
				<Text style={{ color: theme.colors.subText, fontSize: 12 }}>
					Eklendi: {new Date(item.createdAt).toLocaleDateString()}
				</Text>
				<View
					style={[
						styles.statusBadge,
	...
	const renderLogItem = ({ item }) => (
		<View
			style={[styles.logItem, { borderBottomColor: theme.colors.cardBorder }]}
		>
			<View
				style={[
					styles.methodBadge,
			...
			<View style={{ flex: 1 }}>
				<Text style={[styles.logEvent, { color: theme.colors.text }]}>
					{item.event}
				</Text>
				<Text style={[styles.logTime, { color: theme.colors.subText }]}>
		...
		>
			<View style={styles.tabs}>
				...
				<TouchableOpacity
					style={[
						styles.tab,
						activeTab === "logs" && { borderBottomColor: theme.colors.primary },
					]}
			...
			<FlatList
				...
				renderItem={
					activeTab === "endpoints" ? renderEndpointItem : renderLogItem
				}
