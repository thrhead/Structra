...
export function ChartAreaInteractive({ data = [] }: ChartAreaInteractiveProps) {
	...
	return (
		<Card className="@container/card rounded-[2rem] border border-slate-200/60 dark:border-slate-800/60 bg-white/60 dark:bg-slate-900/60 backdrop-blur-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 ease-out flex flex-col group overflow-hidden relative">
			<div
				className="absolute -inset-10 opacity-30 dark:opacity-10 pointer-events-none transition-transform duration-1000 ease-in-out group-hover:scale-105"
				style={{
					background:
						"radial-gradient(ellipse at top right, rgba(99,102,241,0.15), transparent 70%)",
				}}
			></div>
			<CardHeader className="flex flex-col gap-4 space-y-0 p-6 md:flex-row md:items-center relative z-10 border-b border-slate-100/50 dark:border-slate-800/50">
				...
				<CardAction className="flex items-center gap-2">
					...
					>
						<ToggleGroupItem value="90d" className="rounded-lg">
							90 Gün
						</ToggleGroupItem>
						<ToggleGroupItem value="30d" className="rounded-lg">
							30 Gün
						</ToggleGroupItem>
						<ToggleGroupItem value="7d" className="rounded-lg">
							7 Gün
						</ToggleGroupItem>
					</ToggleGroup>
					<Select value={timeRange} onValueChange={setTimeRange}>
						...
						<SelectContent className="rounded-xl border border-slate-200/50 dark:border-slate-800/50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl shadow-xl">
							<SelectItem
								value="90d"
								className="text-xs font-medium cursor-pointer rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
							>
								90 Gün
							</SelectItem>
							<SelectItem
				...
				{filteredData.length === 0 ? (
					<div className="aspect-auto h-[350px] w-full flex flex-col items-center justify-center gap-3 text-slate-400 dark:text-slate-600">
						...
						<p className="text-xs opacity-60">
							Seçili zaman aralığında iş veya maliyet kaydı bulunamadı
						</p>
					</div>
				) : (
					<ChartContainer
