...
const columns: ColumnDef<CustomerData>[] = [
	...
	},
	{
		...
		cell: ({ row }) => (
			<Badge
				variant="outline"
				className="rounded-lg border-slate-100 font-black text-[10px] uppercase"
			>
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			return (
				<div className="text-right font-black italic text-indigo-600">
					{formatted}
				</div>
			);
		},
				...
				<Table>
					<TableHeader className="bg-slate-50/50 dark:bg-slate-800/30">
						{table.getHeaderGroups().map((headerGroup) => (
							<TableRow
								key={headerGroup.id}
								className="hover:bg-transparent border-slate-100 dark:border-slate-800"
							>
								{headerGroup.headers.map((header) => {
									return (
										<TableHead
											key={header.id}
											className="text-[10px] font-black uppercase tracking-widest text-slate-400 py-4"
										>
