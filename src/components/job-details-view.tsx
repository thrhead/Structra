						...
						<CardTitle className="flex justify-between items-start">
							<div className="space-y-1">
								...
								<p className="text-xs text-gray-400">
									KAYIT ID: #{job.id.slice(-6).toUpperCase()}
								</p>
					...
					<CardContent className="space-y-4">
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-sm text-gray-600">
									{format(new Date(job.createdAt), "d MMMM yyyy HH:mm", {
										locale: tr,
									})}
								</p>
							</div>
							...
							<div className="flex items-center gap-3 p-3 bg-amber-50 rounded-lg border border-amber-100">
								...
								<div>
									<p className="font-bold text-sm text-amber-900">
										{job.jobLead?.name || "Lider Atanmamış"}
									</p>
											...
											{assignment.team ? (
												<>
													<div className="flex items-center gap-3 p-2 bg-indigo-50/50 rounded border border-indigo-100 text-sm">
														...
														<span className="text-indigo-900">
															Ekip: <strong>{assignment.team.name}</strong>
														</span>
													</div>
														...
														assignment.team.members.length > 0 && (
															<div className="ml-4 space-y-1">
																...
																<div className="grid grid-cols-1 gap-1">
																	{assignment.team.members
																		.filter(
																			(m) => m.user.id !== job.jobLead?.id,
																		)
																		.map((member, mIdx) => (
																			<div
																				key={mIdx}
																				className="flex items-center gap-2 text-sm text-gray-600 bg-white p-1.5 rounded border border-gray-100 shadow-sm"
																			>
												...
												<div className="flex items-center gap-3 p-2 bg-blue-50/50 rounded border border-blue-100 text-sm">
													...
													<span className="text-blue-900">
														Personel: <strong>{assignment.worker?.name}</strong>
													</span>
												</div>
											)}
										</div>
				...
				<CardContent>
					<div className="space-y-4">
						{job.steps.map((step, index) => {
							const stepNoDisplay =
								step.stepNo || formatTaskNumber(job.jobNo || "JOB", index + 1);
											...
											<div className="flex-1">
												<div className="flex justify-between items-start">
													<div className="flex-1">
														...
														<p
															className={`font-semibold ${step.isCompleted ? "text-gray-900" : "text-gray-600"}`}
														>
															{step.title}
														</p>
															...
															<div className="flex gap-3 mt-1 text-xs text-gray-500 font-medium">
																{hasSubSteps && (
																	<span className="bg-white px-1.5 py-0.5 rounded border border-gray-100">
																		{step.subSteps?.length} alt görev
																	</span>
																)}
																{hasPhotos && (
																	...
																	subStep.photos.length > 0 && (
																		<div className="ml-8 grid grid-cols-4 md:grid-cols-6 gap-2">
																			{subStep.photos.map((photo) => (
																				<div
																					key={photo.id}
																					className="relative aspect-square rounded-md overflow-hidden border border-gray-200 shadow-sm hover:scale-105 transition-transform"
																				>
																					<Image
																						src={photo.url}
														...
														{step.photos?.map((photo) => (
															<div key={photo.id} className="group relative">
																...
																<div className="mt-1.5 flex justify-between items-center px-1">
																	<p className="text-[10px] font-semibold text-gray-600 truncate max-w-[60%]">
						...
						{job.steps.length === 0 && (
							<div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200">
								...
								<p className="text-sm text-gray-500">
									Bu iş için tanımlanmış iş emri (checklist) bulunmuyor.
								</p>
							</div>
						)}
					</div>
