							...
							<div className="flex items-start gap-4 pb-4 border-b">
								...
								<div>
									...
									<p className="text-sm text-gray-500" suppressHydrationWarning>
										{format(new Date(scheduledDate), "d MMMM yyyy, HH:mm", {
											locale: tr,
										})}
									</p>
								</div>
												...
												<div>
													...
													{step.blockedAt && step.blockedReason && (
														<Badge variant="destructive" className="mt-1">
															{BLOCKED_REASONS[step.blockedReason] ||
																step.blockedReason}
														</Badge>
													)}
												</div>
														...
														<div key={subStep.id} className="text-sm">
															<div className="flex items-start gap-2">
																<div
																	className={cn(
																		...
																		subStep.approvalStatus === "APPROVED"
																			? "bg-green-500"
																			: subStep.approvalStatus === "REJECTED"
																				? "bg-red-500"
																				: subStep.isCompleted
																					? "bg-yellow-500"
																					: subStep.blockedAt
																...
																>
																	...
																	{subStep.isCompleted &&
																		subStep.approvalStatus === "PENDING" && (
																			...
																			<div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded">
																				...
																				<div className="flex gap-2">
																					<Button
																						...
																						onClick={() =>
																							handleApprove(subStep.id)
																						}
																						disabled={loadingId === subStep.id}
																					>
																						{loadingId === subStep.id ? (
																		...
																		subStep.photos.length > 0 && (
																			<div className="mt-2">
																				<div className="flex items-center gap-1 text-xs text-gray-500 mb-1">
																					...
																					<span>
																						Fotoğraflar ({subStep.photos.length}
																						)
																					</span>
																				</div>
						...
						{completedDate && (
							<div className="flex items-start gap-4 pt-4 border-t">
								...
								<div>
									...
									<p className="text-sm text-gray-500" suppressHydrationWarning>
										{format(new Date(completedDate), "d MMMM yyyy, HH:mm", {
											locale: tr,
										})}
									</p>
								</div>
					...
					<div className="relative max-w-4xl max-h-[90vh] w-full h-full flex items-center justify-center">
						...
						{selectedPhoto.uploadedBy?.name && (
							<div className="absolute bottom-4 left-4 bg-muted/90 text-white px-3 py-1 rounded text-sm z-10">
								📷 {selectedPhoto.uploadedBy.name} •{" "}
								{format(new Date(selectedPhoto.uploadedAt), "d MMM, HH:mm", {
									locale: tr,
								})}
							</div>
						)}
