...
const DashboardMiniCharts = dynamic(
	() => import("@/components/admin/DashboardMiniCharts"),
	{ ssr: false },
);
...
export default function AdminDashboardClient({
	...
	const topCustomers =
		...
		(data?.latestCustomers || []).map((c: any) => {
			...
			const totalSpent =
				c.jobs?.reduce((sum: number, job: any) => {
					return (
						sum +
						(job.costs?.reduce(
							(jSum: number, cost: any) => jSum + (cost.amount || 0),
							0,
						) || 0)
					);
				}, 0) || 0;
			...
			<section className="space-y-3">
				<div className="px-0.5 flex items-baseline gap-3">
					<div>
						...
						<h3 className="text-lg font-bold tracking-tight text-slate-800 dark:text-slate-100 mt-0.5">
