...
export function GanttChart() {
	...
	const [movingJob, setMovingJob] = useState<{
		id: string;
		initialX: number;
		initialDate: Date;
	} | null>(null);
	...
	const handleDateUpdate = async (jobId: string, newDate: Date) => {
		try {
			...
			if (!job?.scheduledDate || !job.scheduledEndDate) return;

			const diff = newDate.getTime() - new Date(job.scheduledDate).getTime();
			const newEndDate = new Date(
				new Date(job.scheduledEndDate).getTime() + diff,
			);
	...
	return (
		<Card
			className="p-6 bg-card/50 backdrop-blur-sm border-border overflow-hidden"
		...
		>
			...
			<div className="relative border rounded-xl overflow-hidden bg-background">
				...
				<div className="flex border-b">
					<div className="w-48 bg-muted/30 p-4 font-semibold border-r sticky left-0 z-20">
						İşler / Müşteri
					</div>
					<div
							...
							{Array.from({ length: visibleDays }).map((_, i) => {
								...
								return (
									...
									>
										...
										<span
											className={`font-bold ${isToday ? "text-primary" : ""}`}
										>
					...
					{jobs.map((job) => (
						...
						>
							<div className="w-48 p-3 border-r sticky left-0 z-10 bg-background/95 backdrop-blur-sm flex flex-col justify-center gap-1">
								...
								<span className="text-[10px] text-muted-foreground truncate ml-5">
							...
							<div className="flex-1 relative bg-muted/5">
								...
								<div className="absolute inset-0 flex pointer-events-none">
									{Array.from({ length: visibleDays }).map((_, i) => (
										<div
											key={i}
											className="flex-none w-[100px] border-r border-muted/20 last:border-0 h-full"
										/>
									))}
								</div>
								...
								{job.scheduledDate && (
									...
									>
										...
										<span className="text-[10px] font-bold text-primary truncate">
											%{calculateProgress(job)} Tamamlandı
										</span>
									</div>
								)}
							</div>
