...
export function JobApprovalsView({ job }: JobApprovalsViewProps) {
	...
	const pendingSteps = (job?.steps || []).filter(
		(s: any) => s.isCompleted && s.approvalStatus === "PENDING",
	);
	...
	return (
		<div className="space-y-6">
			...
			<Dialog
				open={!!rejectionModal}
				onOpenChange={() => !loading && setRejectionModal(null)}
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>İşi Reddet</DialogTitle>
						<DialogDescription>
							{rejectionModal?.title} seçeneğini reddetmek üzeresiniz. Lütfen
							çalışanın düzeltebilmesi için geçerli bir sebep belirtin.
						</DialogDescription>
					</DialogHeader>
					<div className="py-4">
						<Textarea
						...
						<div className="flex flex-wrap gap-3 mt-2">
							...
							>
								...
								{(() => {
									...
									try {
										return format(new Date(item.completedAt), "PPp", {
											locale: tr,
										});
									} catch (_e) {
										return "-";
