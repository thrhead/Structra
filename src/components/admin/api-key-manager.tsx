...
export function ApiKeyManager() {
	...
	const toggleStatus = async (id: string, currentStatus: boolean) => {
		try {
			...
			setKeys(
				keys.map((k) => (k.id === id ? { ...k, isActive: !currentStatus } : k)),
			);
				...
				<div className="flex items-center gap-4 mb-6">
					...
					<div>
						...
						<p className="text-xs text-muted-foreground">
							Dış sistemler için yetkilendirilmiş anahtarlar oluşturun.
						</p>
					</div>
				</div>
				...
				<form onSubmit={createKey} className="space-y-6">
					<div className="flex gap-4 items-end max-w-md">
						...
						<Button
							type="submit"
							disabled={
								creating || !newName.trim() || selectedScopes.length === 0
							}
						>
					...
					<div className="space-y-3">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
							{AVAILABLE_SCOPES.map((scope) => (
								<div
									key={scope.id}
									className="flex items-center space-x-2 p-2 rounded-lg border border-transparent hover:bg-muted/50 transition-colors"
								>
									<Checkbox
										id={scope.id}
				...
				{newKeyData && (
					<Alert className="mt-6 border-primary/50 bg-primary/5">
						...
						<AlertTitle className="font-bold text-primary">
							Yeni API Anahtarınız Hazır!
						</AlertTitle>
						<AlertDescription className="space-y-4">
							<p className="text-sm mt-2">
								Bu anahtar güvenlik nedeniyle **sadece bir kez**
								gösterilecektir. Lütfen güvenli bir yere kaydedin.
							</p>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				{loading ? (
					...
					keys.map((key) => (
						<Card
							key={key.id}
							className="p-5 border-border hover:border-primary/30 transition-all flex flex-col gap-4"
						>
							<div className="flex justify-between items-start">
								...
								<div
									className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${key.isActive ? "bg-emerald-100 text-emerald-700" : "bg-muted text-muted-foreground"}`}
								>
					...
					<div className="col-span-full p-12 text-center bg-muted/10 rounded-2xl border-2 border-dashed border-muted">
						<p className="text-muted-foreground text-sm">
							Henüz bir API anahtarı oluşturulmamış.
						</p>
					</div>
				)}
			</div>
