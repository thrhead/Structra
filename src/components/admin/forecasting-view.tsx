...
export function ForecastingView() {
	...
	if (loading)
		return (
			<div className="p-8 text-center animate-pulse">{tCommon("loading")}</div>
		);
	...
	return (
		<div className="space-y-6">
			<div className="flex flex-col gap-2">
				...
				<p className="text-muted-foreground">{t("forecastingSubtitle")}</p>
			</div>
			...
			<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
				{forecasts.length === 0 ? (
					...
					forecasts.map((item, idx) => (
						<Card
							key={idx}
							className="overflow-hidden border-border hover:border-primary/50 transition-colors bg-card/50 backdrop-blur-sm shadow-sm"
						>
							...
							<CardContent className="pt-4">
								...
								<div className="mt-4 h-2 w-full bg-muted rounded-full overflow-hidden">
									<div
										...
										style={{
											width: `${Math.min(100, (item.averageDurationHours / 8) * 100)}%`,
										}}
									/>
								</div>
