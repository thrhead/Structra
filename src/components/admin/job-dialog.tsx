...
const jobSchema = z.object({
	title: z
		...
		.min(3, "İş başlığı en az 3 karakter olmalıdır"),
...
export function JobDialog({
		...
		defaultValues: {
			...
			...(job && {
				...
				scheduledEndDate: job.scheduledEndDate
					? new Date(job.scheduledEndDate).toISOString().slice(0, 16)
					: "",
				startedAt: job.startedAt
					? new Date(job.startedAt).toISOString().slice(0, 16)
					: "",
				completedDate: job.completedDate
					? new Date(job.completedDate).toISOString().slice(0, 16)
					: "",
				budget: job.budget,
	...
	useEffect(() => {
		if (job) {
			...
			setValue(
				"startedAt",
				job.startedAt ? new Date(job.startedAt).toISOString().slice(0, 16) : "",
			);
			setValue(
				"completedDate",
				job.completedDate
			...
			if (job) {
				...
				if (result && "error" in result && result.error) {
					if ("errors" in result && result.errors) {
						toast.error(
							`Doğrulama hatası: ${Object.values(result.errors).flat().join(", ")}`,
						);
					} else {
					...
					<FieldGroup>
							...
							<FieldGroup>
								...
								<div className="grid grid-cols-2 gap-4">
									...
									<Field>
										...
										<Input
											id="j-loc"
											{...register("location")}
											placeholder="GPS veya Açık Adres"
										/>
									</Field>
								</div>
								...
								<Field>
									...
									<Textarea
										...
										placeholder="İşe dair detaylı talimatlar..."
						...
						<Fieldset>
							...
							<FieldGroup>
								<div className="grid grid-cols-2 gap-4">
									...
									<Field>
										<FieldLabel htmlFor="j-lead">
											Sorumlu Ekip Lideri
										</FieldLabel>
										<Select
											value={jobLeadId || "none"}
											onValueChange={(val) =>
												setValue("jobLeadId", val === "none" ? null : val)
											}
										>
											...
											<SelectContent>
												...
												{selectedTeam?.members?.map((m) => (
													<SelectItem key={m.user.id} value={m.user.id}>
														{m.user.name}{" "}
														{m.user.id === selectedTeam.lead?.id
															? "(Varsay. Lider)"
															: ""}
													</SelectItem>
												))}
									...
									<Field>
										...
										<Select
											...
											onValueChange={(val: any) =>
												setValue("acceptanceStatus", val)
											}
										>
											<SelectTrigger id="j-acc">
												<SelectValue />
											</SelectTrigger>
											<SelectContent>
						...
						<Fieldset>
							...
							{steps.length === 0 ? (
								<div className="py-8 px-4 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl bg-slate-50/30">
									<FieldDescription>
										Henüz bir adım eklenmedi. İş detaylarını belirlemek için
										yukarıdaki butonları kullanabilirsiniz.
									</FieldDescription>
								</div>
							) : (
								<div className="space-y-4">
									{steps.map((step, index) => (
										<div
											key={index}
											className="relative group/step p-4 rounded-3xl bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200/60 dark:border-slate-800 animate-in fade-in slide-in-from-top-2"
										>
											<div className="flex items-start gap-4">
												...
												<div className="flex-1 space-y-3">
													...
													<div className="space-y-2 pl-4 border-l-2 border-indigo-100 dark:border-indigo-900/50">
														{step.subSteps?.map((subStep, subIndex) => (
															<div
																key={subIndex}
																className="flex items-center gap-2 animate-in slide-in-from-left-2 transition-all"
															>
														...
														<Button
															...
															className="h-8 text-xs font-bold text-indigo-500 hover:text-indigo-600 hover:bg-indigo-50 p-2 rounded-xl"
														>
															<PlusIcon className="h-3 w-3 mr-1" /> Alt Görev
															Ekle
														</Button>
													</div>
												</div>
												...
												<Button
													...
													variant="ghost"
													size="icon"
													className="opacity-0 group-hover/step:opacity-100 text-destructive hover:bg-destructive/10 rounded-xl transition-all"
						...
						<div className="flex justify-end gap-3 mt-8 pt-4 border-t border-slate-100 dark:border-slate-800">
							<Button
								...
								className="rounded-xl min-w-[100px] h-11"
							>
								İptal
							</Button>
							<Button
								type="submit"
								disabled={isLoading}
								className="rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white min-w-[160px] h-11 shadow-lg shadow-indigo-500/20"
							>
								{isLoading ? (
									<>
