				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-xs text-muted-foreground">
							Ekipler için en verimli iş sıralamasını oluşturun.
						</p>
					</div>
				</div>
					...
					<Popover>
						...
						<PopoverContent className="w-auto p-0" align="end">
							<Calendar
								...
								modifiers={{
									hasJobs: (date) =>
										!!calendarData[format(date, "yyyy-MM-dd")]?.total,
								}}
								modifiersClassNames={{
									hasJobs: "font-bold",
								}}
								...
								<div className="flex flex-col gap-1 border-b pb-2">
									<div className="flex items-center gap-2">
										<div
											...
											style={{
												backgroundColor: TEAM_COLORS[rIdx % TEAM_COLORS.length],
											}}
										/>
										<span className="text-sm font-bold text-slate-800">
											{route.teamName}
										</span>
										<span className="text-[10px] font-semibold bg-slate-100 px-1.5 py-0.5 rounded text-slate-600 ml-auto">
											{route.jobs.length} İş
										</span>
									</div>
									...
									{route.jobs.map((job, idx) => (
										...
										>
											...
											{idx !== route.jobs.length - 1 && (
												<div
													className="absolute left-[ -2px ] top-4 bottom-[-16px] w-[2px] bg-slate-200 z-0"
													style={{ left: "-2.5px" }}
												/>
											)}

											<span className="text-[10px] font-bold text-slate-400 shrink-0 mt-0.5 w-4">
												{idx + 1}.
											</span>
