...
function CustomBarTooltip({ active, payload, label }: any) {
	...
	return (
		<div className="rounded-xl border border-slate-200/60 dark:border-slate-800/50 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl shadow-xl p-3 text-xs min-w-[180px]">
			...
			<p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 pt-1 border-t border-slate-100 dark:border-slate-800/50">
...
export default function DashboardMiniCharts({
	...
	const getStatusUrl = (name: string) => {
		if (name.includes("Aktif"))
			return `${basePrefix}/admin/jobs?status=IN_PROGRESS`;
		if (name.includes("Bekleyen")) return `${basePrefix}/admin/approvals`;
		if (name.includes("Tamamlanan"))
			return `${basePrefix}/admin/jobs?status=COMPLETED`;
					...
					{hasWeeklyData ? (
							...
							>
								<BarChart data={weeklyStats} barSize={28}>
									...
									<Tooltip
										...
										cursor={{ fill: "rgba(99,102,241,0.06)", radius: 8 } as any}
									/>
									<Bar
										dataKey="count"
										radius={[8, 8, 0, 0]}
									...
									<div className="space-y-2">
										{selectedDay.jobs.map((job, i) => (
											...
											>
												...
												<p className="text-[11px] font-medium text-slate-600 dark:text-slate-300 truncate mb-1">
													{job.jobTitle}
												</p>
						...
						<div className="h-[200px] flex flex-col items-center justify-center gap-2 text-slate-400 dark:text-slate-600">
							...
							<p className="text-xs font-medium">
								Bu hafta henüz tamamlanan adım yok
							</p>
						</div>
					)}
				</CardContent>
			...
			<div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-3">
				{statsStrip.map((stat) => (
					<Link key={stat.label} href={stat.href} className="block">
						<Card className="rounded-2xl border border-slate-200/60 dark:border-slate-800/50 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 group h-full">
							<CardContent className="p-4 flex items-center gap-3">
								<div
									className={`p-2 rounded-xl ${stat.bg} transition-transform duration-300 group-hover:scale-110`}
								>
									<stat.icon className={`w-4 h-4 ${stat.color}`} />
								</div>
								<div>
									<p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400 dark:text-slate-500 leading-none mb-1">
										{stat.label}
									</p>
									<p
										className={`text-2xl font-bold tabular-nums tracking-tight ${stat.color}`}
									>
										{stat.value}
									</p>
								</div>
