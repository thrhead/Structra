...
import { ArrowUpRight, BarChart2, DollarSign, TrendingUp } from "lucide-react";
...
const CostTrendChart = dynamic(() => import("./charts/CostTrendChart"), {
	ssr: false,
});
const ProjectStatusChart = dynamic(
	() => import("./charts/ProjectStatusChart"),
	{ ssr: false },
);
const WorkloadCostChart = dynamic(() => import("./charts/WorkloadCostChart"), {
	ssr: false,
});
const CompletionChart = dynamic(() => import("./charts/CompletionChart"), {
	ssr: false,
});
const StrategicPulseChart = dynamic(
	() => import("./charts/StrategicPulseChart"),
	{ ssr: false },
);
...
function StatCard({
	...
	return (
		<Card
			className={`group rounded-3xl border border-slate-200/60 dark:border-slate-800/50 bg-white dark:bg-slate-900/80 shadow-sm hover:shadow-xl ${colors.glow} hover:-translate-y-0.5 transition-all duration-300 ease-out flex flex-col justify-between`}
		>
			<CardHeader className="pb-3 px-5 pt-5">
				<div className="flex items-start justify-between">
					<div
						className={`p-2 rounded-2xl border ${colors.iconBg} transition-transform duration-300 group-hover:scale-110`}
					>
			...
			<CardContent className="px-5 pb-5">
				<div
					className={`text-3xl font-bold tabular-nums tracking-tight ${colors.value} leading-none`}
				>
					{value}
				</div>
				{sub && (
...
function ChartCard({
	...
	return (
		<Card
			className={`rounded-3xl border border-slate-200/60 dark:border-slate-800/50 bg-white dark:bg-slate-900/80 shadow-sm overflow-hidden hover:shadow-lg hover:shadow-slate-200/60 dark:hover:shadow-slate-900/40 transition-all duration-300 ${className}`}
		>
...
export default function StrategicView({ data }: { data: any }) {
	...
	const combinedTrendData = useMemo(() => {
		...
		const allDates = Array.from(
			new Set([
				...trends.costs.map((c: any) => c.date),
				...trends.revenue.map((r: any) => r.date),
			]),
		);
		return allDates.sort().map((date) => {
			const revenue =
				trends.revenue.find((r: any) => r.date === date)?.amount || 0;
			const cost = costMap[date] || 0;
			return { date, Gelir: revenue, Maliyet: cost, Kâr: revenue - cost };
		});
	}, [trends]);

	const totalRevenue = (trends?.revenue || []).reduce(
					...
					<Table>
						<TableHeader>
							<TableRow className="hover:bg-transparent border-b border-slate-100 dark:border-slate-800/50">
								...
								<TableHead className="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
									Toplam Bütçe
								</TableHead>
								<TableHead className="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
									Net Kâr
								</TableHead>
								<TableHead className="px-5 py-3 text-center text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
									Marj
								</TableHead>
							</TableRow>
						</TableHeader>
						<TableBody>
							{topCustomersByProfit.map((customer: any, idx: number) => (
								<TableRow
									key={idx}
									className="hover:bg-slate-50/70 dark:hover:bg-slate-800/30 transition-colors duration-150 border-b border-slate-100/60 dark:border-slate-800/30 last:border-0"
								>
									<TableCell className="px-5 py-3.5 font-semibold text-slate-800 dark:text-slate-200 text-sm">
										{customer.customer}
									</TableCell>
									<TableCell className="px-5 py-3.5 text-right text-sm text-slate-500 dark:text-slate-400 tabular-nums">
										₺{customer.budget.toLocaleString("tr-TR")}
									</TableCell>
									<TableCell className="px-5 py-3.5 text-right font-bold tabular-nums text-emerald-600 dark:text-emerald-400 text-sm">
										₺{customer.profit.toLocaleString("tr-TR")}
									</TableCell>
									<TableCell className="px-5 py-3.5 text-center">
										<Badge
											className={`text-[11px] font-semibold rounded-full px-2.5 border-0 ${customer.profitMargin > 20 ? "bg-emerald-100 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400" : "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400"}`}
										>
