	...
	if (active && payload?.length) {
		return (
			<div className="bg-white dark:bg-slate-900 dark:border-slate-800/90 dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-2xl min-w-[160px]">
				<p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-3 border-b dark:border-slate-800 border-slate-100 dark:border-slate-800 pb-2">
					{label}
				</p>
...
const WeeklyStepsChart = memo(
	({ data, categories = [] }: WeeklyStepsChartProps) => {
		...
		if (!isMounted) {
			return (
				<div className="h-[350px] w-full flex items-center justify-center bg-slate-50 dark:bg-slate-900/50 dark:bg-slate-900/50 animate-pulse rounded-3xl">
					Yükleniyor...
				</div>
			);
		}
					...
					{selectedDay ? (
								...
								{selectedDay.jobs && selectedDay.jobs.length > 0 ? (
									...
									<div className="col-span-full py-8 text-center bg-white dark:bg-slate-900 dark:border-slate-800/50 dark:bg-slate-900/20 rounded-xl border border-dashed border-slate-200 dark:border-slate-800">
										<p className="text-xs text-slate-400 font-bold uppercase tracking-widest italic">
											Bu güne ait detaylı iş kaydı bulunamadı.
										</p>
									</div>
								)}
							</div>
						...
						>
							<p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]">
								Grafikteki sütunlara tıklayarak günlük detayları
								inceleyebilirsiniz
							</p>
						</motion.div>
					)}
