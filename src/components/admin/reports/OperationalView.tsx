...
import { Clock, AlertTriangle, CheckCircle2, Zap, ArrowRight, Shield, BarChart3, Timer } from 'lucide-react'
...
export default function OperationalView({ data }: { data: any }) {
    ...
    const totalJobs = Object.values(safeJobStatusDist).reduce((a: any, b: any) => a + b, 0) as number;

    // Separate in-progress and completed bottlenecks
    const inProgressBottlenecks = topBottlenecks.filter((j: any) => j.status === 'IN_PROGRESS');
    const completedBottlenecks = topBottlenecks.filter((j: any) => j.status === 'COMPLETED');

    // SLA health level
    const slaHealth = totalDelayed === 0 && topBottlenecks.length === 0 
        ? 'healthy' 
    ...
    const slaConfig = {
        healthy: { color: 'emerald', icon: CheckCircle2, label: 'SLA Uyumlu', desc: 'Tüm operasyonlar hedef sürelerin dahilinde' },
        warning: { color: 'amber', icon: AlertTriangle, label: 'Dikkat Gerekli', desc: 'Bazı operasyonlarda gecikme tespit edildi' },
        critical: { color: 'rose', icon: AlertTriangle, label: 'Kritik Durum', desc: 'SLA ihlalleri acil müdahale gerektiriyor' }
    }[slaHealth];
            ...
            <div className="grid gap-4 grid-cols-2 lg:grid-cols-4 stagger-children">
                ...
                >
                    <CardHeader className="pb-2 px-5 pt-5">
                        ...
                        <p className="text-[11px] font-semibold uppercase tracking-[0.15em] text-slate-400 dark:text-slate-500 mt-3">SLA İhlalleri</p>
                    </CardHeader>
                ...
                <Card className="group rounded-3xl border border-blue-100/80 dark:border-blue-900/30 bg-white dark:bg-slate-900/80 shadow-sm hover:shadow-xl hover:shadow-blue-500/10 hover:-translate-y-0.5 transition-all duration-300">
                    ...
                    <CardContent className="px-5 pb-5">
                        ...
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Geciken iş oranı</p>
                    </CardContent>
                </Card>
                ...
                >
                    ...
                    <CardContent className="px-5 pb-5">
                        ...
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">
                            {safePendingApprovals.costs || 0} maliyet · {safePendingApprovals.steps || 0} adım
                        </p>
                    </CardContent>
                </Card>
            </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="hover:bg-transparent border-b border-slate-100 dark:border-slate-800/50">
                                ...
                                <TableHead className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Durum</TableHead>
                                <TableHead className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Tahmini</TableHead>
                                <TableHead className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Gerçek</TableHead>
                                <TableHead className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">İlerleme</TableHead>
                                <TableHead className="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-rose-500 dark:text-rose-400">Gecikme</TableHead>
                            ...
                            {topBottlenecks.map((job: any, i: number) => (
                                <TableRow key={i} className="group hover:bg-slate-50/70 dark:hover:bg-slate-800/30 transition-colors duration-150 border-b border-slate-100/60 dark:border-slate-800/30 last:border-0">
                                    <TableCell className="px-5 py-3.5">
                                        ...
                                        <div className="text-xs text-slate-400 dark:text-slate-500 truncate max-w-[200px]">{job.title}</div>
                                        {job.customer && (
                                            <div className="text-[10px] text-slate-400/60 dark:text-slate-500/60 mt-0.5">{job.customer}</div>
                                        )}
                                    </TableCell>
                                    <TableCell className="px-5 py-3.5">
                                        <Badge variant="outline" className={`text-[10px] font-semibold ${
                                            job.status === 'IN_PROGRESS' ? 'border-blue-200 text-blue-600 bg-blue-50 dark:border-blue-800 dark:text-blue-300 dark:bg-blue-950/30' :
                                    ...
                                    <TableCell className="px-5 py-3.5 text-sm text-slate-500 dark:text-slate-400 tabular-nums">
                                        {job.estimatedDuration > 0 ? formatDuration(job.estimatedDuration) : '—'}
                                    </TableCell>
                                    <TableCell className="px-5 py-3.5 text-sm text-slate-500 dark:text-slate-400 tabular-nums">
                                    ...
                                    <TableCell className="px-5 py-3.5">
                                        <div className="flex items-center gap-2">
                                            <Progress value={job.progress || 0} className="h-1.5 w-16 bg-slate-100 dark:bg-slate-800" />
                                ...
                                <TableRow>
                                    <TableCell colSpan={7} className="text-center py-12">
                                        <div className="flex flex-col items-center gap-2 text-slate-400 dark:text-slate-500">
                                            ...
                                            <p className="text-sm font-medium">Kritik gecikme tespit edilmedi</p>
                                            <p className="text-[10px]">Tüm işler tahmini süre dahilinde ilerliyor</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            )}
                                    ...
                                    <div key={i} className="space-y-1.5">
                                        <div className="flex items-center justify-between text-xs">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-semibold text-slate-600 dark:text-slate-300">{STATUS_LABELS[status] || status}</span>
                                            </div>
                    ...
                    <CardContent className="px-5 py-4">
                        <div className="space-y-3">
                            ...
                            <div className={`flex items-center gap-3 p-3.5 rounded-2xl border ${
                                slaHealth === 'healthy' ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-100 dark:border-emerald-900/40' :
                                slaHealth === 'warning' ? 'bg-amber-50 dark:bg-amber-950/20 border-amber-100 dark:border-amber-900/40' :
                                'bg-rose-50 dark:bg-rose-950/20 border-rose-100 dark:border-rose-900/40'
                            }`}>
                                <slaConfig.icon className={`w-5 h-5 flex-shrink-0 ${
                                    slaHealth === 'healthy' ? 'text-emerald-600 dark:text-emerald-400' :
                                    slaHealth === 'warning' ? 'text-amber-600 dark:text-amber-400' :
                                    'text-rose-600 dark:text-rose-400'
                                }`} />
                                <div className="flex-1">
                                    <p className={`font-bold text-sm ${
                                        slaHealth === 'healthy' ? 'text-emerald-700 dark:text-emerald-300' :
                                        slaHealth === 'warning' ? 'text-amber-700 dark:text-amber-300' :
                                        'text-rose-700 dark:text-rose-300'
                                    }`}>{slaConfig.label}</p>
                                    <p className={`text-xs mt-0.5 ${
                                        slaHealth === 'healthy' ? 'text-emerald-600/70 dark:text-emerald-400/70' :
                                        slaHealth === 'warning' ? 'text-amber-600/70 dark:text-amber-400/70' :
                                ...
                                <Link href="/admin/approvals?filter=delayed" className="block">
                                    <div className="flex items-start gap-3 p-3 rounded-xl border bg-rose-50/50 dark:bg-rose-950/10 border-rose-100 dark:border-rose-900/30 hover:border-rose-300 dark:hover:border-rose-700 transition-all group">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between">
                                                <span className="font-semibold text-rose-700 dark:text-rose-300 text-xs">Gecikmiş Onaylar (&gt;48s)</span>
                                            ...
                                            <p className="text-[11px] text-rose-600/70 dark:text-rose-400/60 mt-0.5">
                                                {safePendingApprovals.delayedCosts > 0 && `${safePendingApprovals.delayedCosts} maliyet`}
                                                {safePendingApprovals.delayedCosts > 0 && safePendingApprovals.delayedSteps > 0 && ' · '}
                                                {safePendingApprovals.delayedSteps > 0 && `${safePendingApprovals.delayedSteps} adım`}
                                                {' '}onayı 48 saati aştı
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-start gap-3 p-3 rounded-xl border bg-amber-50/50 dark:bg-amber-950/10 border-amber-100 dark:border-amber-900/30 hover:border-amber-300 dark:hover:border-amber-700 transition-all group">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-[11px] text-amber-600/70 dark:text-amber-400/60 mt-0.5">
                                                {safePendingApprovals.costs || 0} maliyet · {safePendingApprovals.steps || 0} adım onayı bekliyor
                                            </p>
                                        </div>
                                    </div>
                                </Link>
