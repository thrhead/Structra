...
const WeeklyStepsChart = dynamic(() => import("./charts/WeeklyStepsChart"), {
	ssr: false,
});
...
export default function ModernDashboardView({
	...
	const completionRate = Math.round(
		((generalStats.completedJobs || 0) / (generalStats.totalJobs || 1)) * 100,
	);
		...
		<div className="bg-[#f8fafc] dark:bg-slate-950 -m-6 p-8 min-h-screen space-y-8 animate-in fade-in duration-700">
						...
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p className="text-xs text-rose-600/70 dark:text-rose-400/60 font-medium mt-0.5">
									Sistemde {totalPending} adet onay bekleyen kayıt operasyonel
									hızı etkiliyor
								</p>
							</div>
						</div>
			...
			<section className="grid grid-cols-1 md:grid-cols-4 gap-6">
				...
				<div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-[#e2e8f0] dark:border-slate-800 flex items-center gap-4 shadow-sm">
					...
					<div>
						<p className="text-[10px] font-bold text-[#64748b] tracking-wider uppercase">
							AKTİF GÖREV
						</p>
						<h3 className="text-2xl font-black text-[#0f172a] dark:text-white">
							{generalStats.inProgressJobs || 0}
						</h3>
					</div>
				</div>
				...
				<div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-[#e2e8f0] dark:border-slate-800 flex items-center gap-4 shadow-sm">
					...
					<div>
						...
						<h3 className="text-2xl font-black text-[#0f172a] dark:text-white">
							{formatCurrency(generalStats.totalCost || 0)}
						</h3>
					</div>
				</div>
			...
			<section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
				...
				<div className="lg:col-span-1 bg-white dark:bg-slate-900 rounded-xl p-6 border border-[#e2e8f0] dark:border-slate-800 shadow-sm flex flex-col">
					...
					<div className="mb-4">
						<h3 className="text-4xl font-black text-[#0f172a] dark:text-white tracking-tighter">
							{generalStats.completedJobs || 0}{" "}
							<span className="text-xs font-medium text-[#64748b] tracking-normal">
								İş Bitti
							</span>
						</h3>
					</div>
					...
					<div className="flex-1 min-h-[200px] w-full relative overflow-hidden">
						<WeeklyStepsChart
							data={weeklySteps}
							categories={weeklySteps.categories}
						/>
					</div>
				...
				<div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-[#e2e8f0] dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
					<div className="p-6 border-b border-[#e2e8f0] dark:border-slate-800 flex justify-between items-center">
						<div>
							<h3 className="text-lg font-black text-[#0f172a] dark:text-white tracking-tighter uppercase">
								Ekip Performans Matrisi
							</h3>
							<p className="text-[10px] font-bold text-[#64748b] uppercase tracking-widest mt-1">
								Gerçek zamanlı verimlilik sıralaması
							</p>
						</div>
						<div className="flex items-center gap-2 px-3 py-1 bg-[#6366f1]/5 rounded-full text-[#6366f1] border border-[#6366f1]/10">
							...
							<span className="text-[10px] font-black uppercase tracking-widest">
								{teamPerformance.length} AKTİF EKİP
							</span>
						</div>
					</div>
					<div className="flex-1 overflow-auto">
						<table className="w-full text-left border-collapse">
							<thead className="bg-[#f8fafc] dark:bg-slate-800/50 sticky top-0 z-10">
								<tr>
									<th className="px-6 py-4 text-[10px] font-black tracking-widest text-[#64748b] uppercase">
										EKİP ADI
									</th>
									<th className="px-6 py-4 text-[10px] font-black tracking-widest text-[#64748b] uppercase text-right">
										İŞ YÜKÜ
									</th>
									<th className="px-6 py-4 text-[10px] font-black tracking-widest text-[#64748b] uppercase text-center">
										VERİMLİLİK
									</th>
									<th className="px-6 py-4 text-[10px] font-black tracking-widest text-[#64748b] uppercase text-center">
										DURUM
							...
							<tbody className="divide-y divide-[#e2e8f0] dark:divide-slate-800">
								{teamPerformance.slice(0, 10).map((team: any, idx: number) => (
									<tr
										key={idx}
										className="hover:bg-[#f1f5f9]/50 dark:hover:bg-slate-800/30 transition-colors group cursor-default"
									>
									...
									<tr>
										...
										>
											...
											<p className="text-xs font-medium">
												Henüz ekip performans verisi yok
											</p>
										</td>
									</tr>
								)}
