					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-xs text-muted-foreground">
								Olay bazlı anlık veri akışı (HTTP POST) tanımlayın.
							</p>
						</div>
					</div>
			...
			<div className="space-y-3">
				{loading ? (
					...
					webhooks.map((webhook) => (
						<Card
							key={webhook.id}
							className="p-4 border-border flex items-center justify-between group hover:border-primary/30 transition-all"
						>
							<div className="flex items-center gap-4 shrink-0 overflow-hidden">
								<div
									className={`p-2 rounded-full ${webhook.isActive ? "bg-emerald-50 text-emerald-600" : "bg-muted text-muted-foreground"}`}
								>
								...
								<div className="min-w-0">
									...
									<div className="flex items-center gap-2 mt-1">
										...
										<span className="text-[10px] text-muted-foreground italic">
											{format(new Date(webhook.createdAt), "d MMM yyyy", {
												locale: tr,
											})}
										</span>
									</div>
							...
							<div className="flex items-center gap-2">
								...
								<Button
									...
									onClick={() =>
										setDeleteConfirm({ id: webhook.id, open: true })
									}
								>
