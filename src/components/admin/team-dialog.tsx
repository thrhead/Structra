...
import { PlusIcon, Settings2, UserCog, Users } from "lucide-react";
...
export function TeamDialog({
	...
	const leadEligibleUsers = users.filter(
		(u) =>
			u.role === "TEAM_LEAD" ||
			u.role === "WORKER" ||
			u.role === "ADMIN" ||
			...
			<DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto rounded-3xl p-6">
				<DialogHeader>
					...
					<DialogDescription className="sr-only">
						Ekip oluşturma veya düzenleme formu.
					</DialogDescription>
				</DialogHeader>
				<form onSubmit={handleSubmit(onSubmit)} className="mt-6">
					<FieldGroup>
							...
							<FieldGroup>
								<Field>
									...
									<Input
										id="t-name"
										{...register("name")}
										placeholder="Örn: Kuzey Bölge Montaj Ekibi"
										required
									/>
									{errors.name && (
								...
								<Field>
									...
									<Textarea
										...
										placeholder="Ekip görev alanı veya özel notlar..."
						...
						<Fieldset>
							...
							<FieldGroup>
								<Field>
									<FieldLabel htmlFor="t-lead">
										Varsayılan Ekip Lideri
									</FieldLabel>
									<Select
									...
									<FieldDescription>
										Bu kullanıcı sahada yapılacak işlerde varsayılan onay
										mercidir.
									</FieldDescription>
								</Field>
									...
									<div className="border rounded-2xl p-2 space-y-1 max-h-[200px] overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800">
										{workerUsers.length === 0 ? (
											<div className="py-8 px-4 text-center">
												<FieldDescription>
													Sistemde uygun çalışan bulunamadı.
												</FieldDescription>
											</div>
										) : (
						...
						<Fieldset>
							...
							<FieldGroup>
								<Field
									...
									className="p-4 bg-slate-50/50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-900 rounded-2xl justify-between"
								>
									<div className="flex flex-col gap-0.5">
										...
										<FieldDescription>
											Pasif ekipler yeni işlere atanamazlar.
										</FieldDescription>
									</div>
									<Select
						...
						<div className="flex justify-end gap-3 mt-4 pt-2">
							...
							<Button
								type="submit"
								disabled={loading}
								className="rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white min-w-[140px]"
							>
								{loading ? (
									<>
