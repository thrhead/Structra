...
const ConnectorV = ({ isLast }: { isLast?: boolean }) => (
	<div
		className={cn(
			"absolute left-[19px] top-10 border-l-2 border-dashed border-slate-300 -z-10",
			isLast ? "h-6" : "h-full",
		)}
	/>
);
...
const JunctionIcon = ({
	...
	<div className="flex items-center">
		...
		<div
			className={cn(
				"z-10 flex items-center justify-center p-1 rounded-sm bg-white border-2",
				colorClass,
			)}
		>
			<IconShape
				className={cn("w-3 h-3", colorClass.replace("border-", "text-"))}
				fill="currentColor"
			/>
		</div>
		<ConnectorH />
		...
		>
			<div className="flex items-center justify-center w-12 h-full min-h-[50px] bg-white/70 backdrop-blur-sm border-r border-white/40">
				<Icon
					className={cn(
						"w-5 h-5",
						isDarkBg ? "text-slate-800" : "text-slate-600",
					)}
				/>
			</div>
			<div className="px-3 py-2 flex-1 relative z-10">
				...
				<div className="flex justify-between items-center mt-0.5">
					...
					{percentage !== null && (
						<p
							className={cn(
								"text-xs font-bold bg-white/30 px-1.5 rounded",
								isDarkBg ? "text-white" : "text-slate-700",
							)}
						>
							%{percentage}
						</p>
			...
			{hasChildren && (
				<div
					className={cn("pr-2", isDarkBg ? "text-white/80" : "text-slate-400")}
				>
					{isExpanded ? (
	...
	return (
		<div className="relative flex flex-col gap-6 ml-6 mt-2">
			...
			{steps.map((step, idx) => {
				...
				const completedSubs = step.subSteps
					? step.subSteps.filter((s) => s.isCompleted).length
					: 0;
				const percent = hasSubs
					? Math.round((completedSubs / step.subSteps.length) * 100)
					: step.isCompleted
						? 100
						: 0;
		...
		<div className="relative flex flex-col gap-6 ml-6 mt-2">
			{jobs.map((job, idx) => {
				...
				const completedSteps = job.steps
					? job.steps.filter((s) => s.isCompleted).length
					: 0;
				const percent = hasSteps
					? Math.round((completedSteps / job.steps.length) * 100)
					: 0;
							...
							<div key={year} className="relative flex items-start">
								...
								<div className="flex flex-col">
									<div className="flex items-center">
										...
										{isExpanded && (
											<JunctionIcon
												colorClass="border-red-500 text-red-500 rotate-45"
												IconShape={Square}
											/>
										)}
									</div>

									{isExpanded && <JobList jobs={yearJobs} year={year} />}
								</div>
							</div>
						);
					})}
