...
export function SectionCards({ stats }: SectionCardsProps) {
	...
	const premiumCardClass =
		"@container/card rounded-3xl border border-slate-200/60 dark:border-slate-800/50 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 ease-out flex flex-col overflow-hidden relative group";
	...
	const GlowBlob = ({ color }: { color: string }) => (
		<div
			className={`absolute -top-10 -right-10 w-32 h-32 opacity-20 dark:opacity-10 blur-3xl rounded-full transition-transform duration-700 ease-out group-hover:scale-150 ${color}`}
		/>
	);
	...
	return (
		<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 items-stretch relative z-10">
			<Link href={`${basePrefix}/admin/customers`} className="block">
				<Card
					className={`${premiumCardClass} hover:shadow-indigo-500/10 dark:hover:shadow-indigo-500/5 h-full`}
				>
					...
					<CardHeader className="relative z-10">
						...
						<CardAction>
							<Badge
								variant="outline"
								className="border-indigo-200/50 dark:border-indigo-900/50 text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-xl px-2.5 shadow-sm backdrop-blur-md"
							>
			...
			<Link href={`${basePrefix}/admin/teams`} className="block">
				<Card
					className={`${premiumCardClass} hover:shadow-orange-500/10 dark:hover:shadow-orange-500/5 h-full`}
				>
			...
			<Link href={`${basePrefix}/admin/jobs?status=PENDING`} className="block">
				<Card
					className={`${premiumCardClass} hover:shadow-blue-500/10 dark:hover:shadow-blue-500/5 h-full`}
				>
			...
			>
				<Card
					className={`${premiumCardClass} hover:shadow-emerald-500/10 dark:hover:shadow-emerald-500/5 h-full`}
				>
					...
					<CardHeader className="relative z-10">
						...
						<CardTitle
							className={`text-3xl font-bold tabular-nums tracking-tight transition-colors duration-300 ${pColor.text}`}
						>
						...
						<CardAction>
							<Badge
								variant="outline"
								className={`${pColor.border} ${pColor.text} ${pColor.bg} rounded-xl px-2.5 shadow-sm backdrop-blur-md`}
							>
					...
					<CardFooter className="flex-col items-start gap-2 mt-auto relative z-10 pb-6">
						<div
							className={`flex items-center gap-1.5 text-xs font-semibold ${pColor.text}`}
						>
							<IconCheck size={14} stroke={2.5} />{" "}
							{parseFloat(safeStats.completionRate) > 66
								? "Mükemmel başarı oranı"
								: parseFloat(safeStats.completionRate) > 33
									? "İyi gelişim ivmesi"
			...
			<Link href={`${basePrefix}/admin/reports`} className="block">
				<Card
					className={`${premiumCardClass} items-center justify-center p-6 aspect-square max-h-[220px] hover:shadow-teal-500/10`}
				>
						...
						<div className="relative flex items-center justify-center group-hover:scale-105 transition-transform duration-500 ease-out">
							...
							<svg className="transform -rotate-90 w-28 h-28 drop-shadow-sm">
								...
								<circle
									...
									strokeDashoffset={
										251.2 -
										(parseFloat(safeStats.completionRate) / 100 || 0) * 251.2
									}
									strokeLinecap="round"
							...
							<div className="absolute flex flex-col items-center justify-center mb-0.5">
								<span
									className={`text-2xl font-bold tabular-nums tracking-tight ${pColor.text}`}
								>
									{safeStats.completionRate.includes("%")
										? safeStats.completionRate
										: `${safeStats.completionRate}%`}
								</span>
							</div>
