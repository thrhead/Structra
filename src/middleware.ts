...
export default auth((req) => {
	...
	const isAllowed =
		origin && ALLOWED_ORIGINS.some((pattern) => pattern.test(origin));
				...
				return new NextResponse(null, {
					...
					headers: {
						...
						"Access-Control-Allow-Headers":
							"X-CSRF-Token, X-Requested-With, Accept, Accept-Language, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, X-Api-Version, Authorization, X-Platform, X-Client-Version",
	...
	if (isLoggedIn) {
		...
		if (
			...
			return NextResponse.redirect(new URL("/login", req.url));
		if (
			pathWithoutLocale.startsWith("/worker") &&
			!["ADMIN", "MANAGER", "WORKER", "TEAM_LEAD"].includes(userRole!)
		)
			return NextResponse.redirect(new URL("/login", req.url));
