...
export function generateJobPDF(data: JobReportData) {
	...
	doc.setFontSize(10);
	doc.setTextColor(BRAND_COLOR[0], BRAND_COLOR[1], BRAND_COLOR[2]);
	doc.text(`EVRAK NO: #${data.id.substring(0, 12).toUpperCase()}`, 190, 25, {
		align: "right",
	});
	doc.text(
		`DUZENLEME TARIHI: ${format(new Date(), "dd.MM.yyyy HH:mm")}`,
		190,
		32,
		{ align: "right" },
	);
	...
	const summaryData: any[][] = [
		...
		[
			...
			"Durum",
			data.status === "COMPLETED" ? "TAMAMLANDI" : "İŞLEMDE",
		],
		...
		[
			...
			"Tamamlanma",
			data.completedDate
				? format(new Date(data.completedDate), "dd.MM.yyyy")
				: "-",
		],
	];
	...
	for (let i = 1; i <= pageCount; i++) {
		...
		doc.text(
			`Bu rapor Assembly Tracker tarafından güvenli bir şekilde oluşturulmuştur. Tüm hakları saklıdır.`,
			20,
			285,
		);
...
export function generateProfitabilityPDF(data: any[]) {
	...
	yPos += 10;
	doc.setFontSize(10);
	doc.text(`Tarih: ${format(new Date(), "dd.MM.yyyy")}`, 105, yPos, {
		align: "center",
	});
	yPos += 10;
...
export function generateFinancialPDF(summary: any) {
	...
	doc.setFontSize(12);
	doc.text(
		`Toplam Onaylı Harcama: ₺${summary.totalApproved.toLocaleString("tr-TR")}`,
		20,
		yPos,
	);
	yPos += 15;
...
export function generateWorkflowAuditPDF(data: any[]) {
	...
	doc.setFontSize(18);
	doc.text("İŞ AKIŞI DENETİM RAPORU", 105, yPos, { align: "center" });
	yPos += 15;

	data.forEach((job, _index) => {
		if (yPos > 240) {
			doc.addPage();
			yPos = 20;
		}
		...
		const tableData = job.steps.map((step: any) => [
			...
			step.completedBy,
			step.completedAt
				? format(new Date(step.completedAt), "dd.MM HH:mm")
				: "-",
			step.approvedBy,
		...
		autoTable(doc, {
			startY: yPos,
			head: [["Adım", "Bitiren", "Zaman", "Onaylayan", "Süre"]],
			body: tableData,
			theme: "striped",
			headStyles: {
				font: "DejaVuSans",
...
export function generateCostDetailsPDF(data: any[]) {
	...
	doc.setFontSize(18);
	doc.text("DETAYLI MALİYET ANALİZİ", 105, yPos, { align: "center" });
	yPos += 15;
	...
	autoTable(doc, {
		startY: yPos,
		head: [["Tarih", "İş No", "İş Başlığı", "Kategori", "Açıklama", "Tutar"]],
		body: tableData,
		theme: "grid",
		headStyles: {
			font: "DejaVuSans",
