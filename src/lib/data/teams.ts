...
export async function getTeamStats() {
	...
	const members = teamsWithMembers.reduce(
...
async function _getTeamPerformanceStats(teamId: string) {
...
export async function getTeamDetailedReports(teamId: string) {
	...
	const memberStatsMap: Record<
		string,
		{ completedSteps: number; workMinutes: number }
	> = {};
	...
	const monthlyTrend: Record<
		string,
		{ jobCount: number; workHours: number; expenses: number }
	> = {};
	...
	jobs.forEach((job) => {
		...
		job.costs.forEach((cost) => {
			...
			const costMonthKey = format(cost.date || jobDate, "MMM yy", {
				locale: tr,
			});
	...
	const totalStepsCompleted = Object.values(memberStatsMap).reduce(
		(sum, s) => sum + s.completedSteps,
		0,
	);
	const efficiencyScore = Math.min(
		...
		Math.round(
			successRate * 0.7 +
				Math.min(30, totalStepsCompleted / (team?.members.length || 1)),
		),
	);
			...
			memberStats:
				team?.members.map((m) => ({
					...
					workHours: Math.round(
						(memberStatsMap[m.userId]?.workMinutes || 0) / 60,
					),
				})) || [],
		},
	};
...
async function getAllTeamsSummary() {
	...
	const globalStats = {
		...
		totalJobsCompleted: reports.reduce(
			(sum, r) => sum + r.stats.completedJobs,
			0,
		),
		...
		avgEfficiency:
			reports.length > 0
				? Math.round(
						reports.reduce((sum, r) => sum + r.stats.efficiencyScore, 0) /
							reports.length,
					)
				: 0,
	};
