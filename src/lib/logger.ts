import { type LocalLog, offlineDB } from "./offline-db";
...
class Logger {
		...
		try {
			...
			if (typeof window !== "undefined") {
				...
				try {
					...
					console.error(
						"CRITICAL: Server log persistence failed:",
						message,
						dbError,
					);
				}
	...
	error(msg: string, ctx?: any, stack?: string) {
		this.log(LogLevel.ERROR, msg, ctx, stack);
	}
	audit(msg: string, ctx?: any) {
		this.log(LogLevel.AUDIT, msg, ctx);
	}
