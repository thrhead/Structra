...
export async function queueWebhook(
	...
	deliverWebhook(log.id).catch((err) => {
		console.error(
			`[WebhookDeliverer] Initial delivery background error for log ${log.id}:`,
			err,
		);
	});
}
...
export async function deliverWebhook(logId: string) {
	...
	if (!log?.webhook?.isActive) return;
...
async function handleFailure(
	...
	console.warn(
		`[WebhookDeliverer] Delivery failed for log ${logId}. Status: ${status}, Attempts: ${nextAttemptCount}`,
	);
	...
	if (status === "PENDING" && nextAttemptAt) {
		...
		if (delay < 60 * 60 * 1000) {
			// Only setTimeout for delays < 1 hour
			setTimeout(() => {
