...
function generateJobWorkbook(jobData: JobData) {
	...
	jobData.steps.forEach((step) => {
		const subStepsText =
			step.subSteps
				?.map((sub) => `${sub.isCompleted ? "✓" : "○"} ${sub.title}`)
				.join("\n") || "-";
...
function generateJobsListWorkbook(jobs: JobListItem[]) {
	const wb = XLSX.utils.book_new();
...
function generateCostWorkbook(costs: CostData[]) {
	const wb = XLSX.utils.book_new();
	...
	data.push([
		...
		"",
		"GENEL TOPLAM:",
		(totalPending + totalApproved + totalRejected).toString(),
		"",
		"",
	]);
...
function _generateProfitabilityExcel(data: any[]) {
...
function _generateDelayExcel(data: any[]) {
...
export function generateTeamCapacityWorkbook(data: any[]) {
	const wb = XLSX.utils.book_new();
	const headers = [
		["EKİP ADI", "AKTİF İŞ SAYISI", "ÜYE SAYISI", "DOLULUK ORANI (%)"],
	];
	const rows = data.map((d) => [
		d.teamName,
		d.activeJobsCount,
...
function _generateTeamCapacityExcel(data: any[]) {
