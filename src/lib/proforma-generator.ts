...
export function generateProformaPDF(data: ProformaData) {
	...
	doc.text(`Fatura No: #${data.id.slice(-8).toUpperCase()}`, 190, 32, {
		align: "right",
	});
	...
	doc.text(
		`${subTotal.toLocaleString(locale, { minimumFractionDigits: 2 })} ${currencySymbol}`,
		190,
		yPos,
		{ align: "right" },
	);
	...
	doc.text(
		`${taxAmount.toLocaleString(locale, { minimumFractionDigits: 2 })} ${currencySymbol}`,
		190,
		yPos,
		{ align: "right" },
	);
	...
	doc.text(
		`${grandTotal.toLocaleString(locale, { minimumFractionDigits: 2 })} ${currencySymbol}`,
		190,
		yPos + 2,
		{ align: "right" },
	);
	...
	doc.setFontSize(8);
	doc.text(
		"* Ödeme yaparken açıklama kısmına fatura numarasını yazmanız rica olunur.",
		20,
		yPos,
	);
	...
	const footerText =
		"Bu belge dijital olarak Assembly Tracker uzerinden olusturulmustur ve imzaya gerek yoktur.";
