...
export async function createCustomerAction(
	...
	if (!validated.success) {
		throw new Error(
			`Geçersiz veri: ${JSON.stringify(validated.error.flatten())}`,
		);
	}

	const { name, email, password, phone, company, address, taxId, notes } =
		validated.data;
...
export async function updateCustomerAction(
	...
	const { id, name, email, password, phone, company, address, taxId, notes } =
		validated.data;
