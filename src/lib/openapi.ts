...
const options: swaggerJsdoc.Options = {
	definition: {
		...
		info: {
			...
			version: "1.0.0",
			description:
				"[TR] Dış sistem entegrasyonları için Assembly Tracker API dokümantasyonu. [EN] Assembly Tracker API documentation for external system integrations.",
		},
		components: {
			securitySchemes: {
				ApiKeyAuth: {
					...
					name: "X-API-KEY",
					description:
						"[TR] API anahtarınızı buraya girin. [EN] Enter your API key here.",
				},
			},
			schemas: {
				Job: {
					...
					properties: {
						...
						title: {
							type: "string",
							description: "[TR] İş başlığı [EN] Job title",
						},
						status: {
							type: "string",
							enum: ["PENDING", "IN_PROGRESS", "COMPLETED", "CANCELLED"],
							description: "[TR] İş durumu [EN] Job status",
						},
						priority: {
							type: "string",
							enum: ["LOW", "MEDIUM", "HIGH", "URGENT"],
							description: "[TR] Öncelik [EN] Priority",
						},
						customer: {
							type: "string",
							description: "[TR] Müşteri adı [EN] Customer name",
						},
...
function translateSpec(spec: any, lang: "tr" | "en") {
	...
	const translated = json.replace(regex, (_match, tr, en) => {
