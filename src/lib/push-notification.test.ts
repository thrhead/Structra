...
vi.mock("expo-server-sdk", () => {
	function MockExpo() {
		return {
			sendPushNotificationsAsync: vi
				.fn()
				.mockResolvedValue([{ status: "ok", id: "test-id" }]),
	...
	MockExpo.isExpoPushToken = vi
		.fn()
		.mockImplementation((token) => token.startsWith("ExponentPushToken"));
