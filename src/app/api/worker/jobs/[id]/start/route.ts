...
export async function POST(
	...
	try {
		...
		if (conflict) {
			...
			console.warn(
				`[JOB START] Conflict detected for job ${id}. Server: ${job.updatedAt}, Client: ${clientVersion}`,
			);
			return conflict;
		}
		...
		if (!["ADMIN", "MANAGER"].includes(session.user.role)) {
			...
			if (!hasAccess) {
				console.warn(
					`[JOB START] Unauthorized access attempt by ${session.user.email} for job ${id}`,
				);
				return NextResponse.json(
