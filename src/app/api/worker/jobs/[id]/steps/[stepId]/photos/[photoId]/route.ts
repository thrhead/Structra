...
export async function DELETE(
	...
	try {
		...
		try {
			...
			const _publicId = `jobs/${jobId}/${filename.split(".")[0]}`; // Rough estimation
		...
		(async () => {
			try {
				...
				if (job) {
					...
					if (job.creatorId)
						await publishToUser(job.creatorId, "photo:deleted", ablyPayload);
					await broadcast("photo:deleted", ablyPayload);
				}
			} catch (asyncErr) {
				console.error(
					"[Photo Delete] Async post-processing error (non-blocking):",
					asyncErr,
				);
			}
		})();
