...
export async function POST(req: Request) {
	try {
		...
		for (const log of logs) {
			...
			const level = ["DEBUG", "INFO", "WARN", "ERROR", "AUDIT"].includes(
				log.level,
			)
				? log.level
				: "INFO";
			const platform =
				(log.platform as string) ||
				(req.headers.get("user-agent")?.toLowerCase().includes("mobile")
					? "mobile"
					: "web");
		...
		return NextResponse.json(
			{ success: true, count: logs.length },
			{ status: 201 },
		);
	} catch (error) {
