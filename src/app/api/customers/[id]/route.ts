	...
	try {
		...
		if (customer._count.jobs > 0) {
			return NextResponse.json(
				{
					error:
						'Bu müşteriye ait kayıtlı işler (Jobs) mevcut. Geçmiş verilerin korunması için bu müşteri silinemez. Bunun yerine müşteriyi "Pasif" duruma getirmeyi deneyin.',
				},
				{ status: 400 },
			);
		}
		...
		if (error.code === "P2003") {
			return NextResponse.json(
				{
					error:
						"Bu müşteriye bağlı kritik kayıtlar mevcut. Güvenlik nedeniyle bu müşteri doğrudan silinemez.",
				},
				{ status: 400 },
			);
		}
