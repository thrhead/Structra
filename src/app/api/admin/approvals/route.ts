	...
	try {
		...
		switch (type) {
			...
			case "STEP": {
				...
				step.job.assignments.forEach((a) => {
			...
			case "SUB_STEP": {
				...
				const recipients = new Set<string>();
				if (subStep.step.job.creatorId)
					recipients.add(subStep.step.job.creatorId);
				subStep.step.job.assignments.forEach((a) => {
					if (a.workerId) recipients.add(a.workerId);
				});
		...
		return NextResponse.json({
			success: true,
			message: `Successfully ${isApprove ? "approved" : "rejected"}`,
		});
	} catch (error: any) {
