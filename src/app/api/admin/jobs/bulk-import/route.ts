		...
		for (const [title, rows] of jobsMap) {
			try {
				await prisma.$transaction(async (tx) => {
					...
					const companyName =
						rows[0]["Customer Company"] || rows[0].Müşteri || rows[0].Firma;
					if (!companyName) {
					...
					if (!customer) {
						throw new Error(
							`'${companyName}' isimli müşteri bulunamadı. Lütfen önce müşteriyi oluşturun.`,
						);
					}
					...
					const projectNo =
						rows[0]["Project No"] || rows[0]["Proje No"] || null;
