...
export async function POST(req: Request) {
	try {
		...
		const center = fromCenter
			? { lat: centerLat || 41.0082, lon: centerLon || 28.9784 }
			: null;
		...
		if (unassignedJobs.length > 0) {
			const jobsWithCoords = unassignedJobs.filter(
				(j) => j.latitude !== null && j.longitude !== null,
			);
			const optimizedResult = optimizeRouteForTeam(jobsWithCoords, center);
			const withoutCoords = unassignedJobs.filter(
				(j) => j.latitude === null || j.longitude === null,
			);
