...
const updateUserSchema = z.object({
	...
	role: z
		.enum(["ADMIN", "MANAGER", "TEAM_LEAD", "WORKER", "CUSTOMER"])
		.optional(),
	...
	try {
		...
		if (error.code === "P2003") {
			return NextResponse.json(
				{
					error:
						'Bu kullanıcıya bağlı kritik kayıtlar (atamalar, onaylar, masraflar veya mesajlar) mevcut. Güvenlik nedeniyle bu kullanıcı doğrudan silinemez. Bunun yerine kullanıcıyı "Pasif" duruma getirmeyi deneyin.',
				},
				{ status: 400 },
			);
		}
