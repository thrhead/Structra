...
export default function NotificationsPage() {
	...
	}, [status, router.push, fetchNotifications]);
	...
	const getNotificationBgColor = (type: string) => {
		switch (type) {
			...
			default:
				return "bg-indigo-50/50 dark:bg-indigo-950/10 border-indigo-100 dark:border-indigo-900/30";
		}
	};
	...
	const NotificationItem = ({ notification }: { notification: any }) => (
		<div
			onClick={() =>
				handleDeleteNotification(notification.id, notification.link)
			}
			...
			<div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
				<div>
					...
					<p className="text-slate-600 dark:text-slate-400">
						Güncel tüm bildirimleriniz ve duyurular.
					</p>
				</div>
				<div className="flex items-center gap-2">
					...
					<Badge
						variant="secondary"
						className="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-none"
					>
