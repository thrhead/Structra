...
export default function WorkerDashboard() {
	...
	const [filter, setFilter] = useState<"ALL" | "IN_PROGRESS" | "PENDING">(
		"ALL",
	);
	...
	}, [status, session, router.push, fetchJobs]);
	...
	const stats = {
		...
		totalEarnings: jobs.reduce(
			(sum, j) =>
				sum +
				(j.costs?.reduce((s: number, c: any) => s + (c.amount || 0), 0) || 0),
			0,
		),
		pendingApprovals: jobs.reduce(
			(sum, j) =>
				sum + (j.costs?.filter((c: any) => c.status === "PENDING").length || 0),
			0,
		),
	};
					...
					{filteredJobs.length === 0 && !loading && (
						<div className="col-span-full text-center py-12 bg-white rounded-lg border border-dashed">
							...
							<p className="text-gray-500 mt-1">
								Filtreye uygun aktif işiniz bulunmuyor.
							</p>
						</div>
					)}
				</div>
