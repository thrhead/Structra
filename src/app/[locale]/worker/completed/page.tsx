...
export default async function CompletedJobsPage() {
	...
	if (
		!session ||
		!["WORKER", "TEAM_LEAD", "ADMIN"].includes(session.user.role)
	) {
		redirect("/login");
	}
				...
				{jobs.length === 0 && (
					<div className="col-span-full text-center py-12 bg-white rounded-lg border border-dashed">
						...
						<h3 className="text-lg font-medium text-gray-900">
							Henüz tamamlanmış işiniz bulunmuyor
						</h3>
						<p className="text-gray-500 mt-1">
							Tamamladığınız işler burada görünecek.
						</p>
					</div>
				)}
					...
					<Card key={job.id} className="hover:shadow-md transition-shadow">
						...
						<CardContent className="space-y-2 pb-3">
							...
							{job.completedDate && (
								<div className="flex items-center gap-2 text-sm text-gray-600">
									...
									<span>
										{format(new Date(job.completedDate), "d MMMM yyyy", {
											locale: tr,
										})}
									</span>
								</div>
						...
						<CardFooter className="bg-gray-50 p-3">
							<Button asChild className="w-full" variant="default">
								<Link href={`/worker/jobs/${job.id}`}>Detayları Gör</Link>
							</Button>
						</CardFooter>
					</Card>
				))}
