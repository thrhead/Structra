...
export default function JobDetailPage(props: { params: any }) {
	...
	const toggleStep = async (stepId: string, currentStatus: boolean) => {
		...
		if (!currentStatus) {
			...
			if (step.subSteps.some((s) => !s.isCompleted)) {
				toast.warning(
					"Tüm alt görevleri tamamlamadan bu adımı tamamlayamazsınız.",
				);
		...
		try {
			const res = await apiClient.post(
				`/api/worker/jobs/${id}/steps/${stepId}/toggle`,
				{},
			);
	...
	const toggleSubStep = async (stepId: string, subStepId: string) => {
		...
		try {
			const res = await apiClient.post(
				`/api/worker/jobs/${id}/steps/${stepId}/substeps/${subStepId}/toggle`,
				{},
			);

			if (!res.ok) {
				const data = await res.json();
				if (data.error?.includes("fotoğraf")) {
					toast.error(
	...
	const handlePhotoUpload = async (
		...
		// ...
		const url = prompt(
			"Lütfen fotoğraf URL'sini girin (veya boş bırakıp test için rastgele bir resim kullanın):",
		);
		...
		try {
			...
			const res = await apiClient.post(
				`/api/worker/jobs/${id}/steps/${stepId}/photos`,
				{ url: finalUrl },
			);
	...
	const completeJob = async () => {
		if (!id) return;
		const allSubStepsCompleted = job?.steps.every(
			(step) =>
	...
	const handleConfirmComplete = async () => {
		try {
			...
			if (res.ok) {
				...
				const data = await res.json();
				if (data.error?.includes("alt iş emirlerini")) {
					toast.error(
						"bu montajı tamamlayarak kapatmak için tüm alt iş emirlerini tamamlamanız gerekiyor",
					);
				} else {
	...
	const updateStatus = async (newStatus: string) => {
		if (!id) return;
		try {
			const res = await apiClient.patch(`/api/worker/jobs/${id}`, {
				status: newStatus,
			});
											...
											>
												...
												{isBlocked && (
													<div className="text-xs text-red-600 mt-1">
														<span className="font-semibold">Bloklandı:</span>{" "}
														{step.blockedReason}
														{step.blockedNote && (
