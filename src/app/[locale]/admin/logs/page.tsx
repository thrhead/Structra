...
export default function LogsPage() {
	...
	const getLevelBadge = (level: string) => {
		switch (level) {
			...
			case "AUDIT":
				return (
					<Badge className="bg-blue-500 text-white border-none">{level}</Badge>
				);
			case "INFO":
													...
													<TableCell className="py-4">
														<div className="flex flex-col">
															...
															<span className="text-[10px] text-muted-foreground truncate max-w-[150px]">
																{log.user?.email || "automatic@system"}
															</span>
														</div>
													</TableCell>
																					...
																					Object.entries(context).map(
																						([key, val]) => (
																							<Badge
																								key={key}
																								variant="outline"
																								className="text-xs bg-muted/20 font-medium px-2.5 py-0.5 rounded-full border-border lowercase shadow-none text-muted-foreground"
																							>
																								{key}:{" "}
																								{typeof val === "object"
																									? JSON.stringify(val)
