...
function ExportsContent() {
	...
	const downloadCustomReport = async (
		...
		toast.success(
			`${type} raporu ${format.toUpperCase()} olarak hazırlanıyor...`,
		);
	};
				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-sm text-muted-foreground">
							İş raporları ve özel analiz raporları oluşturun.
						</p>
					</div>
				</div>
			</div>
			...
			<Tabs defaultValue="jobs" className="space-y-6">
				<TabsList>
					<TabsTrigger value="jobs" className="gap-2">
						<FileTextIcon className="w-4 h-4" /> Tamamlanan İşler
					</TabsTrigger>
					<TabsTrigger value="custom" className="gap-2">
						<TableIcon className="w-4 h-4" /> Özel Raporlar
					</TabsTrigger>
				</TabsList>
									...
									filteredReports.map((report) => (
										...
										>
											...
											<Button
												variant="outline"
												size="sm"
												onClick={() => downloadJobReport(report.id)}
											>
							...
							{[
								{
									...
									desc: "İş bazlı bütçe ve maliyet karşılaştırması, kâr marjı analizi.",
								},
								{
									id: "financial",
									title: "Finansal Özet Tablo",
									desc: "Genel gider ve bütçe durumu, dönem bazlı özet.",
								},
								{
									id: "cost_details",
									title: "Detaylı Maliyet Analizi",
									desc: "Kategori bazlı harcama detayları ve fatura dökümü.",
								},
							].map((report) => (
								<ReportCard
									key={report.id}
									report={report}
									onDownload={downloadCustomReport}
								/>
							))}
						</div>
					</div>
					...
					<div className="space-y-4">
						...
						<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
							{[
								{
									id: "delay",
									title: "Gecikme ve Darboğaz Analizi",
									desc: "Planlanan vs Gerçekleşen süreler, SLA ihlalleri.",
								},
								{
									id: "workflow_audit",
									title: "İş Akışı Denetimi",
									desc: "Adım bazlı tamamlama süreleri ve onay gecikmeleri.",
								},
							].map((report) => (
								<ReportCard
									key={report.id}
							...
							{[
								{
									...
									desc: "Ekip doluluk ve iş yükü dağılımı, zaman analizi.",
								},
								{
									id: "personnel",
									title: "Personel Performans",
									desc: "Personel bazlı tamamlanan iş sayıları ve kalite puanı.",
								},
							].map((report) => (
								<ReportCard
									key={report.id}
	...
	return (
		<Card className="group hover:border-indigo-200 dark:hover:border-indigo-900 transition-all duration-300 hover:shadow-lg hover:shadow-indigo-500/5 bg-white dark:bg-slate-900/80 rounded-2xl overflow-hidden">
			<CardHeader className="pb-2">
				<CardTitle className="text-base font-bold text-slate-800 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
					{report.title}
				</CardTitle>
			...
			<CardContent className="flex gap-2 pt-2">
				<Button
					size="sm"
					variant="outline"
					className="flex-1 text-[11px] h-8 rounded-xl bg-slate-50/50 dark:bg-slate-800/50"
