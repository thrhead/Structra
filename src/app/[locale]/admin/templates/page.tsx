    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-slate-100">İş Şablonları</h1>
                    <p className="text-gray-500 dark:text-slate-400 mt-2">İş oluştururken kullanılacak şablonları yönetin.</p>
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {templates.map((template) => (
                    <div key={template.id} className="bg-white dark:bg-slate-900 dark:border-slate-800 p-6 rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                            {template.steps.map((step) => (
                                <div key={step.id} className="text-sm border-l dark:border-slate-800-2 border-gray-200 pl-3 py-1">
                ...
                {templates.length === 0 && (
                    <div className="col-span-full text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
                        <p className="text-gray-500 dark:text-slate-400">Henüz şablon bulunmuyor.</p>
