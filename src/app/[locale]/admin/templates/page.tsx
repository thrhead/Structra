		...
		<div className="space-y-6">
			<div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
				<div>
					...
					<p className="text-sm text-slate-400 dark:text-slate-500 mt-1">
						İş oluştururken kullanılacak şablonları yönetin.
					</p>
				</div>
			...
			<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 stagger-children">
				{templates.map((template) => (
					<div
						key={template.id}
						className="group rounded-3xl border border-slate-200/60 dark:border-slate-800/50 bg-white dark:bg-slate-900/80 p-5 shadow-sm hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 flex flex-col"
					>
						...
						<div className="space-y-2 max-h-48 overflow-y-auto pr-2 scrollbar-hide">
							{template.steps.map((step) => (
								<div
									key={step.id}
									className="text-sm border-l-2 border-slate-200 dark:border-slate-700 pl-3 py-1"
								>
				...
				{templates.length === 0 && (
					<div className="col-span-full text-center py-16 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
						...
						<p className="text-sm text-slate-400 dark:text-slate-500 mt-1">
							Excel ile yükleyerek başlayabilirsiniz.
						</p>
					</div>
				)}
			</div>
