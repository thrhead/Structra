				...
				<Card className="border-none shadow-sm">
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{stats.totalWorkingHours} Saat
						</div>
						<p className="text-xs text-muted-foreground">
							Sahada geçirilen toplam süre
						</p>
					</CardContent>
				</Card>
				<Card className="border-none shadow-sm">
								...
								<div className="grid grid-cols-1 gap-4">
									...
									<div className="p-4 border rounded-lg bg-gray-50/50">
										...
										<p className="font-semibold text-sm">
											{format(new Date(team.createdAt), "d MMMM yyyy", {
												locale: tr,
											})}
										</p>
									</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								{team.members.map((member: any) => {
									...
									return (
										<div
											key={member.id}
											className={`flex items-center justify-between p-4 border rounded-lg ${isLead ? "bg-indigo-50 border-indigo-200" : "bg-white dark:bg-slate-900 dark:border-slate-800"}`}
										>
											<div className="flex items-center gap-4">
											...
											<div className="text-right">
												<Badge
													...
													className={
														isLead ? "bg-indigo-600 hover:bg-indigo-700" : ""
													}
												>
							...
							<div className="space-y-4">
										...
										<div className="flex-1">
											...
											<div className="flex items-center gap-4 text-xs text-muted-foreground">
												<span className="flex items-center gap-1">
													<Users className="h-3 w-3" /> {job.customer.company}
												</span>
												<span className="flex items-center gap-1">
													<Clock className="h-3 w-3" />{" "}
													{job.scheduledDate
														? format(new Date(job.scheduledDate), "d MMM yyyy")
														: "Tarih yok"}
												</span>
											</div>
										</div>
								...
								{jobs.length === 0 && (
									<p className="text-center text-muted-foreground py-8 italic">
										Bu ekibe henüz bir iş atanmamış.
									</p>
								)}
							</div>
						</CardContent>
						...
						<Card>
							...
							<CardContent>
								<div className="space-y-4 pt-4">
									...
									<div className="flex justify-between border-b dark:border-slate-800 pb-2">
										<span className="text-muted-foreground">
											İş Başına Ortalama Masraf:
										</span>
										<span className="font-medium">
											₺
											{stats.totalJobs > 0
												? (
														stats.totalExpenses / stats.totalJobs
													).toLocaleString("tr-TR")
												: 0}
										</span>
									</div>
