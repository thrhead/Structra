...
import { BriefcaseIcon, Edit, SearchIcon, UsersIcon } from "lucide-react";
...
import { getTeamStats, getTeams } from "@/lib/data/teams";
...
export default async function TeamsPage(props: {
	...
	const membersMap = new Map(
		teamsWithMembers.map((t) => [t.id, t.members.map((m) => m.userId)]),
	);
		...
		<div className="space-y-6">
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
				<div>
					...
					<h2 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100 mt-0.5">
						Ekipler
					</h2>
				</div>
