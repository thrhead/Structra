...
export default function CustomerJobDetailPage(props: {
				...
				<Card className="border-indigo-200 bg-indigo-50">
					...
					<CardContent className="space-y-4">
						<p className="text-sm text-indigo-800">
							Bu montaj işi ekibimiz tarafından tamamlandı olarak
							işaretlenmiştir. Lütfen yapılan işi kontrol ederek onaylayın veya
							bir eksiklik varsa reddederek ekibe geri bildirimde bulunun.
						</p>
			...
			<Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Lütfen işin neden tamamlanmadığını veya eksik olan kısımları
							belirtin. Bu not saha ekibine iletilecektir.
						</DialogDescription>
					</DialogHeader>
					<Textarea
					...
					<DialogFooter>
						...
						>
							{actionLoading && (
								<CustomSpinner className="h-4 w-4 animate-spin mr-2" />
							)}
							Reddet ve Bildir
						</Button>
					</DialogFooter>
						...
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-sm text-gray-600">
									{format(new Date(job.scheduledDate), "d MMMM yyyy, HH:mm", {
										locale: tr,
									})}
								</p>
							</div>
					...
					{job.completedDate && (
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-sm text-gray-600">
									{format(new Date(job.completedDate), "d MMMM yyyy, HH:mm", {
										locale: tr,
									})}
								</p>
							</div>
							...
							<div className="flex-1">
								<p
									className={cn(
										"text-sm font-medium",
										step.isCompleted
											? "text-green-900 line-through opacity-70"
											: "text-gray-900",
									)}
								>
									{step.title}
							...
							<div key={index} className="space-y-2">
								<div className="flex items-center gap-2">
									...
									<span className="text-xs text-gray-500">
										{format(new Date(approval.updatedAt), "d MMM yyyy", {
											locale: tr,
										})}
									</span>
								</div>
